...
**Schema version:** v33 · **MCP tools:** 27 · **Languages detected:** ~25 (10 AST/parser-tier, 21 regex-tier, plus 1 stub-tier (Haskell) — see [Language support](#language-support))
...
- [The 27 MCP tools](#the-27-mcp-tools)
...
│   └── server/server.go          # 27 MCP tools, HTTP REST, gzip, OpenAPI 3.1, bearer auth,
...
- **Regex gap.** 16 languages still use regex extraction — 9 at the stable 0.85 tier (TS/TSX, Rust, Java, Swift, Kotlin, C#, PHP, C, C++ — Swift/Kotlin/C#/PHP/C/C++ all promoted from 0.70 → 0.85 in v0.73) and 7 at the approximate 0.70 tier (Ruby, Scala, Lua, Zig, Elixir, Dart, R — the latter six promoted from stub in v0.63). 10 languages have parser-backed AST extraction at 1.0 confidence (Go, Python, JS, YAML, JSON, HCL/Terraform, TOML, Bash, Markdown, Jinja2). One language remains stub: Haskell (indentation-sensitive layout makes regex-tier representation significantly harder; tracked under [#1161](https://github.com/kwad77/pincher/issues/1161)). `extraction_confidence` surfaces the tier to callers. Full fix = per-language pure-Go AST libraries (no tree-sitter / no CGO), tracked in the extractor refactor plan; cross-file CALLS resolution for the regex tier is tracked separately under [#1182](https://github.com/kwad77/pincher/issues/1182) (Rust) and [#1183](https://github.com/kwad77/pincher/issues/1183) (Java).
