...
The `_meta.capabilities` field on every tool response includes a tag indicating the binary's source channel — currently `schema_v34` and friends advertise feature support, but a future release may add `release_channel` for symmetry with the install-time signal. For now, `pincher --version` plus the [Releases page](https://github.com/kwad77/pincher/releases) is the canonical signal.
