...
- **Single shared FTS5 corpus**: All symbol kinds (Function, Method, Setting, etc.) share `symbols_fts`, so adding YAML Settings or markdown headings can dilute BM25 scores for code identifiers. Tracked at #32 (per-corpus FTS5 split) — splits into `symbols_fts_code` / `symbols_fts_config` / `symbols_fts_docs`.
