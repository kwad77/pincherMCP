...
- **`internal/db/db.go`** — SQLite store. Schema lives here as a `schema` const. Schema migrations live in `schemaMigrations` (a `[]string` slice — append to add a migration; version is auto-derived from slice length). Current schema: **v10** (v9 = per-corpus FTS5 split — `symbols_code_fts` / `symbols_config_fts` / `symbols_docs_fts` populated alongside legacy `symbols_fts`; #32 part 1, zero observable change. v10 = TOML added to the config-corpus language list — DROPs and recreates the three corpus triggers so existing v9 DBs route TOML symbols correctly). `symSelectFrom` is the canonical SELECT column list used by all symbol queries; update it and all scan functions together when adding columns.
