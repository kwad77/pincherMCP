...
pincher init --target=jetbrains          # .junie/guidelines.md (IntelliJ IDEA, PyCharm, GoLand, WebStorm)
pincher init --target=antigravity        # .agents/rules/pincher.md (Google Antigravity rules)
pincher init --target=antigravity-mcp    # ~/.gemini/antigravity/mcp_config.json (Antigravity MCP server)
...
| **v0.89** | **Eval harness + carryover bugs.** `pincher bench §2` — canonical workflow corpus + comparator implementations ([#1298](https://github.com/kwad77/pincher/issues/1298)), pulled early so the v1.0-launch comparison data lands with runway to act on it. USES_VAR produce-but-not-persist regression ([#1479](https://github.com/kwad77/pincher/issues/1479)). | 🚧 in flight |
