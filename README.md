...
| **v0.20.0** | **JS AST default-on + interface-dispatch dead_code precision + CLI/MCP/HTTP parity.** Tier 1 of multi-language coverage: pure-Go JS AST (#266) promoted from `PINCHER_EXPERIMENTAL_JS_AST=1` to default after fixing four polish bugs (arrow-function double-emit, IsExported semantics, object-literal arrows, locator regex). Interface-dispatch satisfaction analysis closes [#493](https://github.com/kwad77/pincher/issues/493) — methods reachable only via interface dispatch stop showing as dead_code. CLI commands, MCP tools, and HTTP endpoints reach drift-tested parity ([#558](https://github.com/kwad77/pincher/issues/558)). | 🚧 in flight |
...
- **Interface-dispatch dead_code false positives ([#493](https://github.com/kwad77/pincher/issues/493)).** `dead_code` flags methods reachable only via interface dispatch (e.g. an `eval` method satisfying a `whereExpr` interface, called via `w.eval(...)`) — the static graph can't see through interface dispatch. v0.19's receiver-type fix is the prerequisite that's now landed; the satisfaction-analysis pass that closes this is in flight for v0.20 ([interface-impl detection](https://github.com/kwad77/pincher/issues/493)). Mitigation: pair every `dead_code` candidate with a `trace direction=inbound` check before deletion; #492 (Go init/TestMain/main filter) shipped in v0.18.0.
