...
**The retention is the planning-loop surface.** Pincher is the foundation; the `_meta` envelope is the enabler; the LLM in your host's loop is the router. Every response advertises `_meta.capabilities`, `_meta.complexity_tier` (`lite`/`standard`/`heavy`), `_meta.next_steps`, `_meta.empty_reason`, `_meta.warnings_v2`, and stable `X-Request-ID` headers — the signals an agent loop (or a router on top of it) reads to decide what to do next without re-issuing exploratory calls. Pincher is a shared code-intelligence backend that talks both **MCP stdio** (one binary serves every MCP-capable client uniformly) and an **HTTP REST gateway** at `/v1/<tool>` with full OpenAPI 3.1.0 contracts (so a webhook or a curl one-liner reaches the same surface). One backend, one index, one running savings total, every agent in your stack. See [`docs/integrations/loop-leverage-layers.md`](docs/integrations/loop-leverage-layers.md) for the three-layer agent-leverage frame (hook + `_meta` + composite).

Conventional code-search tools index a codebase for humans browsing it through a UI; pincherMCP indexes the same codebase for an LLM agent calling tools. The agent-shaped surface is the whole point — responses sized for a context window rather than a UI pane, runtime interception of Read and Grep calls before the agent opens the file, and a local-only binary so neither the index nor the code leaves the machine.

Under the hood: a single Go binary that indexes the codebase into three co-located layers — byte-offset symbol store, knowledge graph, and FTS5 full-text search — populated in a single AST parse pass from one shared `symbols` table; no duplication, no sync overhead. All three are exposed through **28 agent-callable MCP tools**, every one also reachable via the HTTP REST API at `/v1/<tool>`.
...
| **v0.73** | **Documentation frame + universal regex-tier sweep: Swift / Kotlin / C# / PHP / C / C++ all promoted to 0.85.** Two-axis Phase 3 release. Documentation: migration guide v0.4 → v1.0 reaches review-ready first draft ([#1332](https://github.com/kwad77/pincher/issues/1332)); `docs/integrations/loop-leverage-layers.md` names the three-layer agent-leverage model — hook + _meta + composites ([#1392](https://github.com/kwad77/pincher/issues/1392)); `docs/integrations/meta-envelope-contract.md` names the planning-loop input surface field-by-field ([#1394](https://github.com/kwad77/pincher/issues/1394)); GitHub Pages landing page restructured around the three-leverage-layer frame with worked example + 9-host install matrix ([#1455](https://github.com/kwad77/pincher/issues/1455)) + earlier framing backfill ([#1453](https://github.com/kwad77/pincher/issues/1453)). Extractor sweep: every regex-tier single-language extractor that previously sat at 0.70 — Swift ([#1450](https://github.com/kwad77/pincher/issues/1450)), Kotlin ([#1457](https://github.com/kwad77/pincher/issues/1457)), C# ([#1459](https://github.com/kwad77/pincher/issues/1459)), PHP ([#1461](https://github.com/kwad77/pincher/issues/1461)), C/C++ ([#1463](https://github.com/kwad77/pincher/issues/1463)) — now extracts at 0.85 with full classes/interfaces/enums/structs/actors/records/traits, attribute-prefix syntax, and modifier-order flexibility. Framework extensions: `classMatchedThisLine` suppresses funcRE collision on inline primary constructors (C# records); `enumRE` sets `currentClass` so enum methods bind as Method/Parent (Kotlin enum class); generic type-parameter clauses now support one level of nesting (`<T : Comparable<T>>`); class regex accepts both `[A-Z_]` and `[A-Za-z_]` for C snake_case conventions. The 0.70 tier is now empty for single-language extractors. | ✅ shipped |
| **v0.74** | **Phase 4 prep: composite-tool roadmap + use-case pricing framework + bench composite-score rubric.** Falsifiable-measurement slice. `docs/integrations/composite-tool-roadmap.md` ([#1391](https://github.com/kwad77/pincher/issues/1391)) names the five candidate composites slated for v0.81–v0.85 — `investigate_failure`, `plan_change`, `audit_unused`, `onboard_module`, `why_empty` — with today's atomic-call sequence each replaces and the contract invariants (additive only, no internal MCP round-trips, single envelope, per-composite `_meta.diagnosis_v2` codes, mandatory `empty_reason`, idempotency). `docs/integrations/use-case-pricing.md` ([#1395](https://github.com/kwad77/pincher/issues/1395)) defines the four serious-use-case scenarios (find-dead-code, why-test-failing, blast-radius, orient-on-module) + one negative-control (format-this-file) with three-dimensional measurement methodology (token cost, p50/p95 latency, accuracy via manually labeled reference sets); establishes the per-doc citation policy that prevents future "published 10×, measured 2×" credibility loss. `cmd/pinch/bench_composite.go` ([#1398](https://github.com/kwad77/pincher/issues/1398)) ports Stoa's autoresearch-score pattern with pincher-domain weights (accuracy 10×, round-trips-saved 5×, latency 0.05/0.02, per-token 0.001); default-weights ranking pinned by `TestComposeScore_DefaultWeightsRanking` so weight tuning shows up in PR diffs. `_meta.estimated_cost_usd` ([#1396](https://github.com/kwad77/pincher/issues/1396)) **closed as wontfix** — pincher stays tokens-only per the foundation/enabler/router vocabulary discipline; routers multiply `tokens_used` by their model's cost-per-Ktoken host-side. **Roadmap roll-forward:** four v0.74 items moved out — #1081 → v0.76, #1080 → v0.77, #1298 → v0.78, #1333 → v0.85. | ✅ shipped |
| **v0.75** | **Docs completeness dogfood-3: deployment guides + host tutorials + README framing audit.** Third dogfood-style release dedicated to documentation completeness (matches v0.65 dogfood-1 + v0.68 dogfood-2 cadence). Three coherent doc PRs: `docs/deployment/` grows from one page to four + hub (docker.md / systemd.md / homebrew.md + README hub with decision matrix); `docs/tutorials/` grows from two to five (zed.md / codex.md / jetbrains.md modeled on the existing 6-step shape); README leading-paragraph framing audit per the v0.73 vocabulary discipline ("the retention is routing" → "the retention is the planning-loop surface"; "Sourcegraph / OpenGrok / IntelliJ" → "Conventional code-search tools"). All driven by [#1334](https://github.com/kwad77/pincher/issues/1334). [#1085](https://github.com/kwad77/pincher/issues/1085) (MCP `notifications/message` judgment call) rolled forward to v0.77 to ship in one coherent notification-surface slice alongside [#1080](https://github.com/kwad77/pincher/issues/1080) (`notifications/progress`). | ✅ shipped |
| **v0.76** | **MCP roots multi-root selection + USES_VAR allowlist drift fix.** Two real correctness fixes from a dogfood pass. (1) `pickSessionRoot` iterates every advertised root and filters by `IsBloatTrap(hookMode=true)` — pre-fix pincher took `result.Roots[0]` unconditionally and could land on `/` or `$HOME` if the host advertised them at index 0 ([#1081](https://github.com/kwad77/pincher/issues/1081) Phase 1; Phase 2 multi-root auto-index + `_meta.auto_indexed` deferred to v0.77 with notification surface work). (2) `knownEdgeKinds` allowlists at `cypher/engine.go:494` and `server.go:7133` now include `USES_VAR` — #1380 (v0.71 Ansible Jinja-substitution dataflow edges) shipped the extractor + resolver but missed two consumer-side allowlists, so `MATCH (a)-[:USES_VAR]->(b)` silently returned 0 rows with a misleading "not recognized" warning ([#1480](https://github.com/kwad77/pincher/issues/1480)). | ✅ shipped |
| **v0.77** | **Dogfood-found extractor regressions: Bash + Markdown both produce zero edges in production.** Two real produce-but-not-persist bugs. (1) Bash CALLS dropped because `bashEdgeFromName` returned `""` for top-level invocations; fix emits a per-file Module symbol matching Go's convention and `bashEdgeFromName(scope="")` returns the module name so `nameToID[module]` resolves ([#1482](https://github.com/kwad77/pincher/issues/1482)). (2) Markdown REFERENCES dropped because the link walker used the filename as the QN root but Section symbols use the H1-heading slug; fix derives `selfModule` from `headings[0]` so edges and Sections share the same root ([#1481](https://github.com/kwad77/pincher/issues/1481)). Both bugs shared the same anti-pattern: unit tests asserted extractor output without round-tripping through the resolver. Both fixes ship with new integration probes. Notification surface slice (#1080 / #1085 / #1081 Phase 2) rolled forward to v0.78 to ship cohesively once SDK wiring lands. | ✅ shipped |
| **v0.78** | **Dogfood drain: prune-cascade transient state + Claude Code worktree double-index.** Two more produce-but-not-persist-shape bugs from the overnight filing batch. `list prune_dead=true` carries `_meta.counts_may_be_transient: true` + a warning when the CASCADE delete temporarily collapses surviving projects' counts mid-pass ([#1473](https://github.com/kwad77/pincher/issues/1473)). Walker skips `.claude/` so parent projects no longer double-index Claude Code's `.claude/worktrees/<branch>/` sibling working trees ([#1475](https://github.com/kwad77/pincher/issues/1475)). | ✅ shipped |
| **v0.78.1** | **Patch — force-pass perf + watcher serialize + schema-migration invalidates audit.** CPU profiling reframed the #1474 user report ("force=true rejected with lockfile contention" after schema-migration auto-restart) — 42% of force-pass CPU was SQL parsing in `DeleteSymbolsForFile`'s per-symbol DELETE loop + 42% was `git rev-parse` subprocess invocations bypassed by the force=true branch. Two perf PRs ([#1495](https://github.com/kwad77/pincher/pull/1495) + [#1498](https://github.com/kwad77/pincher/pull/1498)) drop SQL parse share 6× and add a `.git/HEAD`-content cache so rapid-fire force calls share one git invocation; watcher reindexes serially so user `force=true` against any idle project wins the lockfile race immediately. Force pass on pincher-repo: 6.77s → 6.27s median (~7%). Plus the structural audit groundwork via [#1502](https://github.com/kwad77/pincher/pull/1502) — all 32 schema migrations classified by their `invalidates` scope (27 Nothing / 5 All) with the doctor surface live; the gate that suppresses `binaryDriftForce` when migrations-applied union to Nothing is the v0.80 follow-up. Plus `retry_rate` → `zero_result_rate` metric-honesty rename ([#1499](https://github.com/kwad77/pincher/pull/1499)), Ansible USES_VAR drop-reason diagnostic counters ([#1500](https://github.com/kwad77/pincher/pull/1500)), README/REFERENCE Known limitations audit ([#1501](https://github.com/kwad77/pincher/pull/1501)). v0.78.0 binary catch-up: [#1085](https://github.com/kwad77/pincher/issues/1085) MCP `notifications/message` + [#1081](https://github.com/kwad77/pincher/issues/1081) Phase 2 multi-root auto-index actually landed in master before the v0.78.0 release-prep merge — the v0.78.0 binary ships them; v0.78.1's CHANGELOG is the catch-up. | ✅ shipped |
| **v0.79** | **Hardening: capability-advertisement audit (9 doc + invariant gates) + bench-baseline refresh workflow + slog convention.** Pre-stable-promotion `.x9` cycle per [#672](https://github.com/kwad77/pincher/issues/672). Nine PRs add structural gates so the same drifts can't recur on v0.80 stable: capability-table parity ([#1505](https://github.com/kwad77/pincher/pull/1505)), tool-count anchor parity ([#1506](https://github.com/kwad77/pincher/pull/1506)), env-var doc parity + 3 missing rows ([#1508](https://github.com/kwad77/pincher/pull/1508)), HTTP endpoint parity + `/v1/ready` row ([#1510](https://github.com/kwad77/pincher/pull/1510)), CLAUDE.md required-gates list ([#1511](https://github.com/kwad77/pincher/pull/1511)), cypher `t.Skip`→`t.Fatal` ([#1512](https://github.com/kwad77/pincher/pull/1512)), `slog`-only production logging ([#1513](https://github.com/kwad77/pincher/pull/1513)), tool-listing name parity + `context_for_task` row ([#1514](https://github.com/kwad77/pincher/pull/1514)). Workstream-2 bench-baseline refresh ships as a manual-dispatch workflow ([#1504](https://github.com/kwad77/pincher/pull/1504)) so the gate is enforceable for the first time since v0.71 (8 minors of silent drift). [#1337](https://github.com/kwad77/pincher/issues/1337) cross-platform install harness + [#1389](https://github.com/kwad77/pincher/issues/1389) cross-language FP sweep stay open as continuing pre-1.0 work, not v0.79 blockers. | ✅ shipped |
| **v0.80** | **Stable promotion.** Same binary as v0.79; new release channel only (Homebrew bump + Docker `latest`/`stable` tags). No new features or behavior changes between the v0.79 final and v0.80 tag — that's what STABLE means in pincher's release-channel scheme ([docs/release-channels.md](docs/release-channels.md)). | ✅ shipped |
| **v0.81** | **Phase 4 composite #1: `investigate_failure`.** Bug-hunt loop in one call ([#1517](https://github.com/kwad77/pincher/pull/1517)). Takes raw stack trace, parses frame tokens (Go panics, Python tracebacks, Node/Java traces), BM25-searches each across the code corpus, and ranks suspects by weighted evidence: stack-frame match + file-in-trace + recent-change + caller fan-in. Returns `{implicated_symbols, callers, recent_changes, rank}` with per-suspect `evidence` enumerating which signals fired so the agent can trust the rank or pivot. Replaces the canonical 5-call bug-hunt sequence (search × N → trace × N → changes overlap → context) with one round-trip. Read-only; no internal MCP round-trips per the composite contract. First of 5 composites planned for Phase 4; `plan_change` rolls forward to v0.82 per the [composite-tool roadmap](docs/integrations/composite-tool-roadmap.md). | ✅ shipped |
| **v0.82** | **Phase 4 composite suite complete + v1.0 plan refresh + dogfood-found composite hardening.** Megarelease: `plan_change` ([#1519](https://github.com/kwad77/pincher/pull/1519)) + `audit_unused` ([#1546](https://github.com/kwad77/pincher/pull/1546)) + `onboard_module` ([#1548](https://github.com/kwad77/pincher/pull/1548)) + `why_empty` ([#1549](https://github.com/kwad77/pincher/pull/1549)) close the 5/5 Phase 4 composite roadmap. FILE-A through FILE-V scaffold the v0.83–v0.97 path to 1.0 — 22 v1.0-blocker issues either shipped, scaffolded with phased rollout, or recorded as ADR-deferred decisions (ADR-0002 v1.0 frozen surface, ADR-0003 plugin-extractor deferral). Six dogfood-found composite-hardening patches ([#1583](https://github.com/kwad77/pincher/pull/1583)–[#1588](https://github.com/kwad77/pincher/pull/1588)) surfaced in the same session by reading shipped composite source via `mcp__pincher__context` — empty-query hint, bare-filename routing, wrong empty_reason code, cancellation contract gap, non-deterministic error response, silent partial-scope drop. Umbrella [#1582](https://github.com/kwad77/pincher/issues/1582). | ✅ shipped |
| **v0.83** | **Dogfood-reserve patch slot + v0.83-milestone backlog drain.** Four v0.83-milestone issues closed: Go toolchain bump to 1.25.10 ([#1594](https://github.com/kwad77/pincher/pull/1594)) closing six stdlib CVEs the govulncheck advisory gate flagged; MCP stdio loop treats stdin-EOF as graceful shutdown ([#1593](https://github.com/kwad77/pincher/pull/1593)) so host-conformance replay no longer drops the final response; `context_for_task` cancellation contract + correct `target_not_resolved` empty_reason ([#1592](https://github.com/kwad77/pincher/pull/1592)) closing the gap v0.82's FILE-H sweep missed. Plus follow-on `investigate_failure` + `onboard_module` inner-loop ctx checks ([#1596](https://github.com/kwad77/pincher/pull/1596)) caught by reading shipped composite source. Returns to the one-theme-per-release cadence the v0.82 megarelease broke. | ✅ shipped |
| **v0.84** | **API freeze checkpoint** ([ADR-0002](docs/adr/0002-v1-frozen-surface.md)). Every PR touching a frozen surface element from this minor onward either reverts or routes to 2.x. Schema-migration language-scoped selective invalidation ([#1543](https://github.com/kwad77/pincher/issues/1543)) — only files of the listed languages re-extract on binary upgrade. `_meta.empty_reason` orphan-stamp drain ([#1603](https://github.com/kwad77/pincher/issues/1603)) — `low_confidence_extractor`, `unsupported_language`, `stale_index` all now stamped with dedicated `next_steps`; audit moves 8 → 12 stamped (`same_file_only` is the last remaining orphan). FILE-H cancellation contract becomes uniform across every atomic tool — `handleIndex` joins the rest in returning O(microseconds) on a pre-cancelled context. MCP capability-surface expansion ([#1080](https://github.com/kwad77/pincher/issues/1080) notifications/progress, [#1082](https://github.com/kwad77/pincher/issues/1082) guide-as-Prompt) deferred to v1.x — pre-soak surface stays minimal. | ✅ shipped |
| **v0.85** | **Perf observability + usage-frequency levers (axis-A measurement, axis-B redirect).** Per-phase / per-language / per-resolver / per-tool wall-clock timing across the indexing pipeline ([#1613](https://github.com/kwad77/pincher/issues/1613), [#1620](https://github.com/kwad77/pincher/pull/1620), [#1625](https://github.com/kwad77/pincher/pull/1625), [#1628](https://github.com/kwad77/pincher/pull/1628), [#1640](https://github.com/kwad77/pincher/pull/1640)) — first dogfood data shows extraction is 82% of force-reindex cost and 73% of THAT is DB-write serialization, redirecting v0.86 perf work from resolve-parallelism to writer-pool contention. Plus axis-B usage-frequency levers: `pincher doctor` advisory when the PreToolUse hook is NOT installed ([#1636](https://github.com/kwad77/pincher/pull/1636)), `queries_zero_result` split into audit-shape vs caller-surprised exposing the actionable failure rate ([#1633](https://github.com/kwad77/pincher/pull/1633)), in-conversation `next_steps_adherence` telemetry on every response ([#1639](https://github.com/kwad77/pincher/pull/1639)), `trace` reverse-direction suggestions on empty-leaf hops ([#1638](https://github.com/kwad77/pincher/pull/1638)). Doctor nested-project advisory suppresses intentional testdata corpus fixtures ([#1647](https://github.com/kwad77/pincher/pull/1647)). | ✅ shipped |
| **v0.86** | **Writer-pool contention fix + PreToolUse hook converted to advisory.** Headline `db.Store.CommitFileExtraction` ([#1627](https://github.com/kwad77/pincher/issues/1627)) collapses four per-file transactions into one — cuts ~2358 writer-mutex acquisitions per 786-file force-reindex pass and strengthens atomicity. Hook becomes advisory ([#1654](https://github.com/kwad77/pincher/issues/1654)) — never blocks Read/Grep, only nudges via `systemMessage`; Grep follows the same contract ([#1656](https://github.com/kwad77/pincher/issues/1656)); prose / planning files exempted; size floor 4 KB → 16 KB. Plus v0.85 dogfood polish: `search` exact-identifier wildcard fallthrough ([#1643](https://github.com/kwad77/pincher/issues/1643)), BY TOOL render width fit ([#1645](https://github.com/kwad77/pincher/issues/1645)), test-file exemption ([#1646](https://github.com/kwad77/pincher/issues/1646)), Cursor hook-check equivalent ([#1336](https://github.com/kwad77/pincher/issues/1336)), hook-stats SQL counts `redirect_advisory` ([#1658](https://github.com/kwad77/pincher/issues/1658)). | ✅ shipped |
| **v0.87** | **Incremental resolve + dogfood-driven advisory hardening.** The resolve pass now scopes to changed files + their cross-file referrers on watcher ticks ([#1629](https://github.com/kwad77/pincher/issues/1629)) — a leaf-file edit's resolve block runs ~87ms vs ~660ms project-wide (7.6×). Go READS predeclared-identifier pre-filter ([#1622](https://github.com/kwad77/pincher/issues/1622)), opt-in persistent Python subprocess ([#1626](https://github.com/kwad77/pincher/issues/1626)), memory-pressure watcher backoff ([#1572](https://github.com/kwad77/pincher/issues/1572)), FTS5 fragmentation advisory ([#1612](https://github.com/kwad77/pincher/issues/1612)). Dogfood-found: four-class branch-drift advisory false-positive suppression ([#1665](https://github.com/kwad77/pincher/issues/1665)–[#1671](https://github.com/kwad77/pincher/issues/1671)), FTS5 threshold tune ([#1663](https://github.com/kwad77/pincher/issues/1663)), `guide` filler-word fix ([#1682](https://github.com/kwad77/pincher/issues/1682)). | ✅ shipped |
| **v0.88** | **Host conformance + per-host docs.** Host-conformance corpus extended from one host to all six — cursor / codex / jetbrains / vscode-copilot / zed join claude-code ([#1562](https://github.com/kwad77/pincher/issues/1562)–[#1566](https://github.com/kwad77/pincher/issues/1566)). Per-host benefits pages in `docs/integrations/` ([#1393](https://github.com/kwad77/pincher/issues/1393)). Python AST persistent subprocess promoted to default-on with a committed bench gate measuring ~126× steady-state amortisation ([#1685](https://github.com/kwad77/pincher/issues/1685)). Ships **zero new MCP-protocol surface** — the MCP capability trio (resources/completion/icons, [#1083](https://github.com/kwad77/pincher/issues/1083)) and Swift AST ([#1452](https://github.com/kwad77/pincher/issues/1452)) were deferred to v1.1 so v1.0 freezes a soaked surface, not an expanded one. | ✅ shipped |
| **v0.89** | **Hardening before stable: cross-language extractor-correctness sweep + composite output bounds + CI race coverage.** The `.x9` pre-promotion hardening minor ([#674](https://github.com/kwad77/pincher/issues/674)). The [#1389](https://github.com/kwad77/pincher/issues/1389) cross-language false-positive sweep drained six extractor mis-type/collision classes across C / C++ / C# / Go ([#1693](https://github.com/kwad77/pincher/issues/1693)) plus a `qualified_name_collision` overload carve-out; a follow-on dogfood pass across twelve more languages caught three more — Java `record` mis-typed as Method ([#1735](https://github.com/kwad77/pincher/issues/1735)), Scala `trait` double-extracted ([#1737](https://github.com/kwad77/pincher/issues/1737)), Ruby `def self.X` named `self` ([#1739](https://github.com/kwad77/pincher/issues/1739)). The three composite tools `plan_change` / `investigate_failure` / `context_for_task` were emitting 200–460 KB unbounded responses past the MCP token cap — all three now bound their caller lists ([#1727](https://github.com/kwad77/pincher/issues/1727) / [#1723](https://github.com/kwad77/pincher/issues/1723) / [#1729](https://github.com/kwad77/pincher/issues/1729)); `dead_code`/`audit_unused` no longer flag Go `init()` as safe-to-delete ([#1722](https://github.com/kwad77/pincher/issues/1722)); `onboard_module`'s cross-project JOIN leak fixed ([#1725](https://github.com/kwad77/pincher/issues/1725)). New advisory `-race` CI job ([#1733](https://github.com/kwad77/pincher/issues/1733)) — CI had zero data-race coverage on a concurrent codebase — immediately caught two test-only races ([#1741](https://github.com/kwad77/pincher/issues/1741) / [#1743](https://github.com/kwad77/pincher/issues/1743)). Multi-platform install-validation harness ([#1337](https://github.com/kwad77/pincher/issues/1337)); host-conformance replay repaired end-to-end ([#1706](https://github.com/kwad77/pincher/issues/1706) / [#1718](https://github.com/kwad77/pincher/issues/1718)); `go-sdk` v1.4.0 → v1.4.1 closing two security advisories ([#1704](https://github.com/kwad77/pincher/issues/1704)); Yarn Plug'n'Play artifacts blocklisted ([#1731](https://github.com/kwad77/pincher/issues/1731)). `pincher bench §2` eval harness — canonical workflow corpus + raw-Read/Grep and `ctags` comparators with tokens/accuracy/latency ([#1298](https://github.com/kwad77/pincher/issues/1298)). USES_VAR produce-but-not-persist regression guard ([#1479](https://github.com/kwad77/pincher/issues/1479)). | ✅ shipped |
| **v0.90** | **Stable promotion.** Same binary as the v0.89 final; new release channel only (Homebrew bump + Docker `latest`/`stable` tags). | planned |
...
- **Edge graph: cross-file CALLS resolution covers Go, Python, and TS/JS receiver-method calls; every other extracted language has same-file CALLS only.** As of v0.62 ([#1159](https://github.com/kwad77/pincher/pull/1159)) every regex-tier language emits same-file CALLS edges via the shared `regexCallScan` path. v0.57 ([#856](https://github.com/kwad77/pincher/issues/856)) put Python on the cross-file resolver alongside Go. v0.72 ([#1177](https://github.com/kwad77/pincher/issues/1177)) defers TS/JS CALLS with non-empty `ReceiverType` for cross-file resolution — bare free-function calls still drop on per-file miss to bound noise from regex-tier extraction. **Cross-file resolution for Rust and Java** is tracked under [#1182](https://github.com/kwad77/pincher/issues/1182) / [#1183](https://github.com/kwad77/pincher/issues/1183) — AST-extractor work gated on a tree-sitter-vs-pure-Go decision, milestoned v1.0. The graph-shaped tools (`trace`/`dead_code`/`neighborhood`/`context`) emit a per-language honesty warning when the dominant language lacks cross-file resolution ([#1145](https://github.com/kwad77/pincher/issues/1145) / [#1146](https://github.com/kwad77/pincher/issues/1146)).
- **Binary-version bump triggers full project re-extract.** A schema-version migration in a new pincher binary invalidates every project's last-indexed schema stamp, so the next `Index()` pass treats every file as dirty regardless of whether the migration actually changed any specific language's extractor. Practical cost: ~25–30s per project on `make install` + `/mcp` reconnect for a project of ~700 files. v0.78.1 reduces the per-pass cost — subquery bulk deletes in `DeleteSymbolsForFile` cut SQLite parse-time CPU share from ~42% to ~10%, the watcher reindexes projects serially so only one cross-process lockfile is held at a time, and `force=true` skips redundant `git rev-parse` invocations when `.git/HEAD` is unchanged ([#1495](https://github.com/kwad77/pincher/pull/1495) / [#1498](https://github.com/kwad77/pincher/pull/1498)). The per-migration "affected extractor set" filter that would let unchanged languages skip re-extraction entirely is tracked under [#1497](https://github.com/kwad77/pincher/issues/1497).
...
> **Recently resolved:** v0.89 (shipped 2026-05-20) is the `.x9` pre-promotion hardening minor before the v0.90 stable promotion ([#674](https://github.com/kwad77/pincher/issues/674)). The [#1389](https://github.com/kwad77/pincher/issues/1389) cross-language extractor-correctness sweep drains 98 `qualified_name_collision` false positives across C / C++ / C# / Go ([#1693](https://github.com/kwad77/pincher/issues/1693)) plus three more dogfood-found extractor bugs (Java `record` mis-typed as Method, Scala `trait` double-extracted, Ruby `def self.X` named `self`). The three caller-tracing composites — `plan_change` / `investigate_failure` / `context_for_task` — now bound their output below the MCP per-call token cap (they previously emitted 200–460 KB responses the host could not receive). A new advisory `-race` CI job adds the data-race coverage the concurrent codebase never had, and immediately caught and cleared two test-only races. Plus a multi-platform install-validation harness, a `go-sdk` v1.4.1 security bump, host-conformance replay repaired end-to-end, and the `pincher bench §2` eval harness completed with raw-Read/Grep and `ctags` comparators. v0.88 (shipped 2026-05-19) extends the host-conformance corpus to all six supported MCP hosts, adds per-host benefits pages under `docs/integrations/`, and promotes the Python AST persistent subprocess to default-on (~126× steady-state amortisation, committed bench gate). It deliberately ships zero new MCP-protocol surface — the resources/completion/icons capability trio and the Swift AST extractor were deferred to v1.1 so v1.0 freezes a soaked surface rather than an expanded one. v0.87 (shipped 2026-05-19) makes the watcher's incremental tick fast — the resolve pass scopes to changed files + their cross-file referrers, so a leaf-file edit's resolve block runs ~87ms vs ~660ms project-wide ([#1629](https://github.com/kwad77/pincher/issues/1629)) — and hardens the `doctor` advisory surface against false positives (a four-class branch-drift suppression cascade, FTS5-fragmentation-advisory threshold tune). v0.86 (shipped 2026-05-19) collapsed the per-file extraction transactions four-into-one to kill writer-pool contention and converted the PreToolUse hook to advisory mode so it never blocks Edit-prep workflows. v0.85 (shipped 2026-05-19) shipped the per-phase / per-language / per-resolver observability that drove the v0.86–v0.87 perf work. v0.84 (shipped 2026-05-19) is the **API freeze checkpoint** ([ADR-0002](docs/adr/0002-v1-frozen-surface.md)). Schema-migration **language-scoped selective invalidation** ([#1543](https://github.com/kwad77/pincher/issues/1543)) lands the headline feature: a binary upgrade running migrations classified as `MigrationInvalidates.Languages` (not `All`) now re-extracts only files of the listed languages — every other file keeps its hash and skips. The `_meta.empty_reason` orphan-stamp audit drains from 8 → 12 stamped: `low_confidence_extractor` ([#1609](https://github.com/kwad77/pincher/pull/1609)), `unsupported_language` ([#1610](https://github.com/kwad77/pincher/pull/1610)), `stale_index` ([#1616](https://github.com/kwad77/pincher/pull/1616)) each get dedicated stamp sites in `search`'s empty branch with cause-specific `next_steps`. `same_file_only` is the last remaining orphan (deferred — requires same-file caller detection inside `trace`). The FILE-H cancellation contract becomes uniform: `handleIndex` joins the rest of the atomic standard-tier surface with an entry-point `ctx.Err()` check ([#1617](https://github.com/kwad77/pincher/pull/1617)) — surfaced as a Windows CI flake where the pre-Index work blew the 500ms contract bound. MCP capability-surface expansion ([#1080](https://github.com/kwad77/pincher/issues/1080) notifications/progress, [#1082](https://github.com/kwad77/pincher/issues/1082) guide-as-Prompt) deferred to v1.x — pre-soak surface stays minimal. v0.83 (shipped 2026-05-19) drains the v0.83 milestone and follow-on dogfood: MCP stdio loop now treats stdin-EOF as graceful shutdown ([#1593](https://github.com/kwad77/pincher/pull/1593)) so host-conformance replay no longer drops the final response; Go toolchain bumped to 1.25.10 ([#1594](https://github.com/kwad77/pincher/pull/1594)) closing six stdlib CVEs (html/template XSS via dashboard renderer, crypto/x509 chain-build amplification via HTTPS fetch, net NUL-byte panic on Windows); `context_for_task` cancellation contract + correct `target_not_resolved` empty_reason ([#1592](https://github.com/kwad77/pincher/pull/1592)) closing the gap v0.82's FILE-H sweep missed; `investigate_failure` + `onboard_module` inner-loop ctx checks ([#1596](https://github.com/kwad77/pincher/pull/1596)) caught by reading shipped composite source. v0.82 (shipped 2026-05-19) is the megarelease: the Phase 4 composite suite (`plan_change`/`audit_unused`/`onboard_module`/`why_empty`) now ships 5/5; FILE-A through FILE-V scaffold the v0.83–v0.97 path to 1.0; ADR-0002 declares the v1.0 frozen surface; six dogfood-found composite-hardening patches close pre-soak gaps. v0.79 (shipped 2026-05-18) is the pre-stable-promotion hardening cycle per [#672](https://github.com/kwad77/pincher/issues/672) — no user-facing feature gaps closed, but nine doc-parity and test-invariant gates land so the next stable release ships with structural protection against drifts that previously took 6–8 release cycles to surface. The `_meta.capabilities` surface gets full forward parity (every advertised tag is now both runtime-probed and doc-table-documented, [#1505](https://github.com/kwad77/pincher/pull/1505)); `docs/REFERENCE.md` and `docs/tutorials/*.md` get count + name + anchor parity tests against the runtime tool set + non-tool HTTP routes ([#1506](https://github.com/kwad77/pincher/pull/1506) / [#1510](https://github.com/kwad77/pincher/pull/1510) / [#1514](https://github.com/kwad77/pincher/pull/1514)); env-var docs gain a tutorial-forward parity check ([#1508](https://github.com/kwad77/pincher/pull/1508)); the slog-everywhere convention gets a production-code scan ([#1513](https://github.com/kwad77/pincher/pull/1513)); four cypher tests that PASS-as-skipped on silent regression are tightened to t.Fatal ([#1512](https://github.com/kwad77/pincher/pull/1512)); CLAUDE.md required-gates list catches up to the actual workflow ([#1511](https://github.com/kwad77/pincher/pull/1511)); the workstream-2 manual-dispatch bench baseline workflow ([#1504](https://github.com/kwad77/pincher/pull/1504)) re-enables a perf-regression gate that had drifted unenforced across 8 feature minors. v0.78 (shipped 2026-05-18) drains two more dogfood-found bugs from the overnight filing batch: `list prune_dead=true` carries `_meta.counts_may_be_transient: true` + warning when the CASCADE temporarily collapses surviving projects' counts mid-pass ([#1473](https://github.com/kwad77/pincher/issues/1473)); walker skips `.claude/` so parent projects no longer double-index Claude Code's `.claude/worktrees/<branch>/` sibling working trees ([#1475](https://github.com/kwad77/pincher/issues/1475)). v0.77 (shipped 2026-05-18) fixes two dogfood-found extractor regressions where the per-extractor unit tests passed but production lost 100% of the edges: Bash CALLS dropped at top-level because `bashEdgeFromName` returned `""` and the indexer's per-file `nameToID` couldn't resolve empty FromQN ([#1482](https://github.com/kwad77/pincher/issues/1482)), Markdown intra-doc REFERENCES dropped because the link walker used the filename as the QN root but Section symbols use the H1-heading slug ([#1481](https://github.com/kwad77/pincher/issues/1481)). Both ship with integration probes that round-trip through extract → resolve → persist so the production-failure shape can't recur silently. v0.76 (shipped 2026-05-18) lands two real correctness fixes from a dogfood pass — `pickSessionRoot` now iterates every advertised MCP root and filters by `IsBloatTrap(hookMode=true)` so pincher no longer picks `/` or `$HOME` just because the host advertised it at `result.Roots[0]` ([#1081](https://github.com/kwad77/pincher/issues/1081) Phase 1; Phase 2 deferred to v0.77 with notification surface work); the two hardcoded `knownEdgeKinds` allowlists at `cypher/engine.go:494` and `server.go:7133` now include `USES_VAR` so `MATCH (a)-[:USES_VAR]->(b)` resolves instead of returning 0 rows with a misleading "not recognized" warning ([#1480](https://github.com/kwad77/pincher/issues/1480) — same drift class as the open #1477 JS AST drift + #1328 family). v0.75 (shipped 2026-05-18) ships the docs-completeness dogfood-3 slice — `docs/deployment/` grows from one page (helm) to four pages plus a hub (docker.md / systemd.md / homebrew.md / README hub with decision matrix); `docs/tutorials/` grows from two pages (claude-code / cursor) to five (zed.md / codex.md / jetbrains.md modeled on the existing 6-step shape); README leading-paragraph framing audit per the v0.73 vocabulary discipline ("retention is routing" → "retention is the planning-loop surface"; competitor name-drops swapped for "conventional code-search tools"). All driven by [#1334](https://github.com/kwad77/pincher/issues/1334). v0.74 (shipped 2026-05-18) ships the Phase 4 prep slice — three coherent artifacts the v0.81–v0.85 composite-tool implementations land against: `docs/integrations/composite-tool-roadmap.md` ([#1391](https://github.com/kwad77/pincher/issues/1391)) names five composites with contract invariants; `docs/integrations/use-case-pricing.md` ([#1395](https://github.com/kwad77/pincher/issues/1395)) defines four serious-use-case scenarios + a negative-control and the per-doc citation policy preventing future "published 10×, measured 2×" credibility loss; `cmd/pinch/bench_composite.go` ([#1398](https://github.com/kwad77/pincher/issues/1398)) ports Stoa's autoresearch-score with pincher-domain weights and a default-ranking gate test. `_meta.estimated_cost_usd` ([#1396](https://github.com/kwad77/pincher/issues/1396)) closed as wontfix — pincher stays tokens-only per the foundation/enabler/router discipline. v0.73 (shipped 2026-05-17) drains the regex-tier 0.70 bucket — Swift ([#1450](https://github.com/kwad77/pincher/issues/1450)), Kotlin ([#1457](https://github.com/kwad77/pincher/issues/1457)), C# ([#1459](https://github.com/kwad77/pincher/issues/1459)), PHP ([#1461](https://github.com/kwad77/pincher/issues/1461)), C/C++ ([#1463](https://github.com/kwad77/pincher/issues/1463)) all promoted to the 0.85 stable-regex tier with full class / interface / enum / struct / actor / record / trait coverage, attribute-prefix syntax (Swift `@`, C# `[Attribute]`, PHP `#[Attribute]`), modifier-order flexibility, and (where applicable) generic-clause nesting. The 0.70 tier is now empty for single-language extractors. Three Phase 3 docs items ship together: migration guide v0.4 → v1.0 review-ready first draft ([#1332](https://github.com/kwad77/pincher/issues/1332)), `docs/integrations/loop-leverage-layers.md` naming the three-layer agent-leverage frame ([#1392](https://github.com/kwad77/pincher/issues/1392)), `docs/integrations/meta-envelope-contract.md` naming the planning-loop input surface field-by-field ([#1394](https://github.com/kwad77/pincher/issues/1394)). GitHub Pages landing page complete restructure around the three-leverage-layer frame ([#1455](https://github.com/kwad77/pincher/issues/1455)). v0.72 (shipped 2026-05-17) completes the strict-cross-project guard ([#1232](https://github.com/kwad77/pincher/issues/1232)) follow-through across every retrieval-shape tool — `symbol`/`context` ([#1408](https://github.com/kwad77/pincher/issues/1408)), `neighborhood` ([#1425](https://github.com/kwad77/pincher/issues/1425)), `trace` ([#1431](https://github.com/kwad77/pincher/issues/1431)) — so the canonical `search → symbol` workflow no longer breaks on hosts that have indexed forks of the same source tree. pinchQL grows two long-standing surface gaps: `WHERE col IN [a, b, c]` ([#1439](https://github.com/kwad77/pincher/issues/1439)) and `COUNT(DISTINCT n.prop)` ([#1437](https://github.com/kwad77/pincher/issues/1437)). Two Go-extractor shadow fixes ([#1423](https://github.com/kwad77/pincher/issues/1423) READS, [#1429](https://github.com/kwad77/pincher/issues/1429) CALLS) drain phantom-edge false positives when a local name matches a project Function. Schema **v33** adds `extraction_failures.binary_version_at_failure` so doctor consumers can tell fixed-since-this-binary rows from still-recurring rows without cross-referencing CHANGELOG ([#1421](https://github.com/kwad77/pincher/issues/1421)). One BREAKING (pre-1.0): `projects.current_branch` JSON tag renamed to `last_indexed_branch` ([#1388](https://github.com/kwad77/pincher/issues/1388)) across every response carrying a project record. v0.71 (shipped 2026-05-17) closed the silent "checkout without re-index → wrong byte-offsets" footgun — schema v32 adds `projects.current_branch` plus a `branch` column on `symbols`/`edges`/`files` (v31); the indexer stamps git branch on every Symbol/Edge it writes, and `doctor` surfaces a branch-drift advisory when on-disk branch differs from last-indexed ([#1303](https://github.com/kwad77/pincher/issues/1303) Phase 1 + 2a; Phase 2b PK widening rolls to v0.72 as [#1371](https://github.com/kwad77/pincher/issues/1371)). Ansible YAML closes BOTH legs of [#71](https://github.com/kwad77/pincher/issues/71) in one cycle — Phase 1 `INCLUDES` playbook→role + `LOADS` host→`host_vars` ([#1160](https://github.com/kwad77/pincher/issues/1160)) AND Phase 2 `USES_VAR` Jinja-substitution dataflow edges to canonical `Setting` declarations in `group_vars/` / `host_vars/` / `vars/` / role defaults ([#1165](https://github.com/kwad77/pincher/issues/1165)) — alongside Bash/HCL/Markdown/Makefile structural edges ([#1341](https://github.com/kwad77/pincher/issues/1341)–[#1344](https://github.com/kwad77/pincher/issues/1344)). Synthetic external Module symbols at sentinel `@external/<qn>` keep unresolved IMPORTS edges from silently dropping at resolve time ([#1340](https://github.com/kwad77/pincher/issues/1340)). Structured `_meta.warnings_v2` + `_meta.diagnosis_v2` ship side-by-side with legacy string forms for typed-envelope MCP hosts ([#1098](https://github.com/kwad77/pincher/issues/1098)). Typed-SDK codegen scaffolding under `sdks/` with openapi-generator configs for TS/Python/Go ([#1262](https://github.com/kwad77/pincher/issues/1262)). MCP `StartSchemaDriftWatcher` exits on DB-vs-binary schema-version skew so supervised mode respawns ([#1374](https://github.com/kwad77/pincher/issues/1374)). `pincher init --target=jetbrains` lands JetBrains AI Assistant rules at `.idea/.junie/guidelines.md` ([#1335](https://github.com/kwad77/pincher/issues/1335)). New `pincher hook-stats --export-7d` CLI emits a shareable JSON snapshot of trailing 7-day hook conversion metrics for the [#640](https://github.com/kwad77/pincher/issues/640) field-data thread (closes [#662](https://github.com/kwad77/pincher/issues/662)). v0.70 stable promotion tagged 2026-05-17 with full bug-fix triage + perf regression bar + cross-platform smoke + capability audit green. v0.69 collapsed the v0.60 perf-regression surface that had stacked across Phase 2 feature releases — watcher no-change tick 4806 → 716 allocs/op (-85%, 5.5× faster); `_meta.tokens_used` cl100k BPE → char/4 heuristic by default (opt back in via `PINCHER_TOKEN_ACCOUNTING=exact`) closing every handler bench below the v0.60 baseline. Full release-by-release history in [`CHANGELOG.md`](CHANGELOG.md); long-form rationale per item in [REFERENCE.md → Known limitations](docs/REFERENCE.md#known-limitations).
