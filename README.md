...
| **v0.15.0** | **Autoresearcher dogfood loop + entry-tool depth.** Supervised mode pushes `notifications/tools/list_changed` after respawn so clients re-list new tools live ([#407](https://github.com/kwad77/pincher/issues/407)); pinchQL `WHERE n.id="X"` actually pushes to SQL instead of post-filtering a 400-row LIMIT ([#412](https://github.com/kwad77/pincher/issues/412)); `guide` deepens to task-shape-aware recommendations ([#397](https://github.com/kwad77/pincher/issues/397)); HTTP gateway gzip middleware. | 🚧 in flight |
