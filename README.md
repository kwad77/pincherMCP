...
| **v0.18.0** | **Edge-graph convergence + receiver-type tracking.** Atomic project-wide cross-file edge replace so incremental indexing converges on rule changes without `--force` ([#475](https://github.com/kwad77/pincher/issues/475)); function-typed field call resolution via receiver-type tracking ([#423](https://github.com/kwad77/pincher/issues/423)); honest-savings polish — `baseline_method` stamp in `_meta` ([#477](https://github.com/kwad77/pincher/issues/477)). | 🚧 in flight |
...
- **Pre-#465 polymorphic-method CALLS edges persist after upgrade ([#475](https://github.com/kwad77/pincher/issues/475)).** v0.16.0 stopped new bare-name `String` / `Error` / `Read` / etc. method resolution from creating false-positive edges. Existing DBs upgraded to v0.16.0 / v0.17.0 carry the old edges until force-reindexed. Recommended migration after upgrading: run `pincher index <path> --force` once per project to re-extract symbols + edges from scratch. Atomic project-wide edge replace (so incremental re-index converges automatically) is tracked for v0.18.0.
