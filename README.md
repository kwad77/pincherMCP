...
| **v0.59** | **Hardening + stability checks (Phase 1 pre-promotion).** No new features — bug-fix triage, perf regression validation vs v0.51.1 baseline, cross-platform install smoke, capability-advertisement audit, v0.60 stable-promotion sign-off, Phase 2 backlog build. Targeted bugs: ghost-advisory low-edges-per-symbol ratio extension ([#1010](https://github.com/kwad77/pincher/issues/1010)), watch-triggered reindex ~30% symbol-count gap after binary drift ([#986](https://github.com/kwad77/pincher/issues/986)), JSON test-fixture bloat heuristic ([#996](https://github.com/kwad77/pincher/issues/996)), hook conversion-rate field measurement ([#640](https://github.com/kwad77/pincher/issues/640)). Umbrella: [#664](https://github.com/kwad77/pincher/issues/664). | 🚧 in flight |
...
> **Recently resolved:** v0.58 closed the silent-cross-project-leak class across every retrieval-shape tool (#1049–#1052) — agents probing a symbol id on a multi-project install no longer get bytes from a mirror-project copy without a warning. Ghost-extraction silent-zero-row diagnosis closed across `dead_code`/`architecture`/`schema`/`query` (#1040–#1044) — empty results now explain themselves when the project has symbols but no edges. v0.57 closed the dead_code false-positive triangle's final leg via the type-info-gated resolver (#760), Python full IMPORTS+CALLS edge resolution (#856), and C same-file CALLS edges (#858 first step). Plus the older fixes still live: dead_code FP triangle legs 1–3 (#423 v0.19, #493 v0.20, #565/#576 v0.21+v0.22) and polymorphic-method CALLS edge persistence from pre-v0.17 (#475). After upgrading, run `pincher index <path> --force` once to pick up edges from extractor improvements. Full release-by-release history in [`CHANGELOG.md`](CHANGELOG.md); long-form rationale per item in [REFERENCE.md → Known limitations](docs/REFERENCE.md#known-limitations).
