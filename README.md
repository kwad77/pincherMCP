...
| **v0.21.0** | **Dead_code FP triangle's last leg + parity finish-line.** Function values bound to struct fields ([#565](https://github.com/kwad77/pincher/issues/565)) — the dominant remaining dead_code FP class on Go projects (~70% of post-v0.20 candidates on pincher-repo). Build-tag duplicate-implementation siblings ([#566](https://github.com/kwad77/pincher/issues/566)). `Run` added to polymorphic-method blocklist to defang `cmd.Run()` (`*exec.Cmd`) false-binds ([#567](https://github.com/kwad77/pincher/issues/567)). Plus #558 phases 2-4: HTTP endpoints for `doctor`/`rebuild-fts`/`self-test`, cross-surface parity test, InputSchema-derived OpenAPI bodies. | 🚧 in flight |
| **v0.30.0** | **Dashboard hardening.** Test foundation (E2E harness, HTML snapshot, mobile viewport) + API hardening (pagination, error envelope, ETA) + UX polish (per-tab error state, dark mode, keyboard shortcuts, empty-state CTA, 30+ items total). Tracking umbrella: [#519](https://github.com/kwad77/pincher/issues/519). | planned |
...
- **Function-value-as-field dead_code false positives ([#565](https://github.com/kwad77/pincher/issues/565)).** Methods or functions bound to a struct field (e.g. `s.field = s.method` then `s.field(...)`, or `T{Handler: someFn}` struct literals, or slice-of-structs function entries) are silently flagged dead because the static graph sees `s.field()` as a call to the field, not to the bound function. Third leg of the dead_code FP triangle after #423 (receiver-type field-method) and #493 (interface dispatch) — accounts for ~70% of remaining `dead_code` candidates on pincher-repo as of v0.20.0. Tracked for v0.21.0. Mitigation: pair `dead_code` candidates with `trace direction=inbound` AND a manual scan for the function name in struct-literal contexts before deletion.
