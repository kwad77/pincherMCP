...
| **v0.70** | **Stable promotion** — full bug-fix triage + perf regression bar + cross-platform smoke + capability audit + sign-off doc all green. Tracks [#670](https://github.com/kwad77/pincher/issues/670). | 🚧 in flight |
...
> **Recently resolved:** v0.69 collapses the v0.60 perf-regression surface that had quietly stacked across the Phase 2 feature releases. The watcher's 2s no-change tick had silently regressed 331 → 4806 allocs/op since v0.60 because the cross-file resolve block + PythonSourceRoots WalkDir ran unconditionally on every poll; gated on `force || totalFiles > 0`, the path is back to 716 allocs/op (-85%, 5.5× faster) — pincher's background watcher is finally cheap. The companion regression on the server side was `BenchmarkAuth_TimingProfile/correct` running cl100k_base BPE inside `_meta.tokens_used` accounting on every API call (60% of allocations); switching to a char/4 heuristic by default (opt back in via `PINCHER_TOKEN_ACCOUNTING=exact`) closes every handler bench — Symbol / Search / Query / Architecture / Symbols batch all now resolve below the v0.60 baseline. Windows users get a Scoop install path via `scoop install https://raw.githubusercontent.com/kwad77/pincher/master/packaging/scoop/pincher.json` (#1260 §1); Mac users running `pincher update` after `brew install pincher` now get dispatched to `brew upgrade pincher` instead of the useless `go install` fallback (#1260 §5); ADR-0001 records the deferred-to-v1.0 code-signing decision with bypass documented in packaging/README.md for v0.x direct downloads (#1260 §2). `pincher doctor --fix` shipped earlier in the cycle for the safe-action allowlist (currently VACUUM-when-bloated; explicit-action subcommands still required for destructive remediations). internal/index coverage 84.2% → 86.1% via test-only progress-helper coverage (#1164 §1). HTML extractor bug found by `pincher doctor` against pincher's own `docs/index.html` — `bytesFindHeadingAfter` returning 0 for both "match at offset 0" and "not found" produced zero-byte-range Section symbols on multi-element headings; -1 sentinel + skip in the caller fixed cleanly. `adr action=export` renders the project's runtime ADR map as Markdown for piping into `docs/adr/`, bridging the runtime ADR storage with the cross-release pattern established by ADR-0001 (#1331 §1). Four Mermaid architecture diagrams committed at `docs/architecture/` covering storage-layers / indexer-pipeline / mcp-stack / watcher-lifecycle (#1331 §2). Cold-path resolve gets a threshold-gated bulk QN preload for large projects (#1338); OpenAPI 3.1.0 spec-validity test gate ahead of typed-SDK generation (#1262); branch-aware git hook no-op signals (#1303 §2a) skip reindex on file checkouts + same-branch re-checkouts. Capability + tool-description opt-outs (#1087/#1088) for heavy-traffic aggregators that want to skip per-call envelope tokens. v0.68 ships `pincher bench` — the runs-on-your-own-project token-savings measurement (subcommand + `--persist` flag + `GET /v1/bench-results` HTTP endpoint + dashboard Bench History panel rendering predicted-vs-actual savings% per tool per project). Synthetic pincher-repo numbers are easy to dismiss; `pincher bench` on your codebase is the local proof. Closure phase 2 ([#685](https://github.com/kwad77/pincher/issues/685)) lands `via_kind` on closure rows (schema v30) so the trace fast-path populates `Via` identically to the CTE path — the v0.54 phase-1 empty-Via trade-off is closed. `BuildClosure` now filters source edges to the default trace kind set, making closure data semantically equivalent to CTE for default-kinds queries. The [#1162](https://github.com/kwad77/pincher/issues/1162) closure-tables-default-on decision shipped: re-measured against four 10k-40k file corpora post-#685, real-world mean speedup is 2.3-5.6× (10× p50 bar not met on real corpora), so closure stays opt-in via `PINCHER_CLOSURE_TABLES=1` for users with massive call graphs where the win matters. `pincher init --git-hooks` ([#1261](https://github.com/kwad77/pincher/issues/1261) §1) installs post-checkout/post-merge/post-rewrite hooks so branch switches trigger an eager reindex instead of leaving the index in a mixed-branch state. `traces_otlp` capability now has a runtime probe — a future regression silently demoting the live exporter back to noop fails CI loudly. OTLP tracer coverage push 12.5% → 87.5%. v0.67 closes the #635 dashboard panel triad (per-tool call breakdown, per-tier complexity, payload-size distribution, Tool-Mix Health entropy) — substrate that's been buffering since v0.64's `session_tool_calls` event log finally renders. Both insights propagate to CLI via doctor advisories (payload outliers, tool-mix stuck-loop) so non-dashboard users see the same signal. OTLP traces complete #1163's observability story — per-tool-call spans + per-index-pass spans + Error status on `res.IsError` + graceful shutdown, plus a dashboard Backend Status strip + `health.observability` field surfacing metrics/SSE/OTLP state. Multi-language extractor scoping lands the `scopeRE` shared mechanism — Rust `impl` and Swift `extension` blocks now bind methods to receiver type; Kotlin extension functions capture real method name (not receiver type); #1134 `range over receiver.Field` infers element type. Search gains a `snippet_lines` knob with query-aware default (exact-identifier queries → 0 lines, lexical queries → 5 lines). CONTRIBUTING + troubleshooting docs ([#1264](https://github.com/kwad77/pincher/issues/1264)) close the community-surface gap, with five new `area:*` labels applied across 18 open issues. `cmd/tracelatencybench` ships as the acceptance gate for #1162's closure-tables-default-on decision (rolls to v0.68). v0.66 shipped three things at once: the thin-client payload umbrella ([#1224](https://github.com/kwad77/pincher/issues/1224)) — `trace`/`search` `compact=true`, `_meta=lite` envelope, `trace max_hops` cap, `neighborhood include_fixtures`, all delivering ~30-50% per-call payload reduction + ~150-200 tokens off every response envelope in lite mode; silent-cross-project guard ([#1232](https://github.com/kwad77/pincher/issues/1232)) — `symbol`/`context`/`neighborhood` now error on omitted-project requests whose ID lives only off-session (`cross_project=true` opt-in preserves legacy silent-fallback); plus a 16-fix DOGFOOD drain including `doctor` latency 60s→<2s on multi-project DBs ([#1205](https://github.com/kwad77/pincher/issues/1205)), per-project byte estimate ([#1220](https://github.com/kwad77/pincher/issues/1220)), Markdown preamble extraction so pincher can finally find its own README tagline ([#1097](https://github.com/kwad77/pincher/issues/1097)), `pincher vacuum` 4-step flow ([#1219](https://github.com/kwad77/pincher/issues/1219)), parity-check guard for #1231 ([#1233](https://github.com/kwad77/pincher/issues/1233)/[#1234](https://github.com/kwad77/pincher/issues/1234)), and #1232/#1097/#1224 umbrella close. v0.65 ([#1194](https://github.com/kwad77/pincher/pull/1194)–[#1200](https://github.com/kwad77/pincher/pull/1200)) drained the description-honesty surface across seven tools — `search` / `architecture` / `query` / `fetch` / `stats` / `symbol` / `doctor` — pinning description-vs-runtime parity with contract tests. The highest-impact catch was `fetch`'s `search kind:Document` recommendation (FTS5 operator syntax), which was silently misleading agents into wide BM25 queries — agents now get the correct `kind="Document"` arg-style syntax. v0.64 ([#1189](https://github.com/kwad77/pincher/pull/1189) / [#1190](https://github.com/kwad77/pincher/pull/1190) / [#1191](https://github.com/kwad77/pincher/pull/1191) / [#1192](https://github.com/kwad77/pincher/pull/1192)) lands the dashboard data plumbing (schema v27 `session_tool_calls` per-call event log, substrate for tool-call entropy + payload distribution + per-tier saved-percentage medians) plus a description-honesty audit drain — `dead_code` no longer claims `language=Go` is a default, `health` distinguishes three parser tiers AST/Regex/Stub. python-web pinned corpus exercises FastAPI/Django-shaped Python (decorators + inheritance + async) end-to-end. v0.63 ([#1186](https://github.com/kwad77/pincher/pull/1186) / [#1187](https://github.com/kwad77/pincher/pull/1187)) promoted 6 of 7 stub-tier languages (Lua / Elixir / Zig / Scala / Dart / R) to regex-tier — only Haskell remains stub. v0.62 ([#1159](https://github.com/kwad77/pincher/pull/1159)) closed the edge-graph-empty warning surface for every regex-tier language. v0.61 ([#1158](https://github.com/kwad77/pincher/pull/1158)) lit up TypeScript Method extraction + per-file CALLS + per-language polymorphic blocklist. v0.60 drained the silent-confidently-wrong family across pinchQL warning surfaces, drift-correctness, and encoder consistency. Watch-triggered partial reindex stuck behind a new-binary stamp (#986) now atomic — interrupted passes stay drift-retriable instead of locking 30% symbol coverage. Binary-version downgrade race across concurrent same-schema processes (#1154) now guarded by a Go-side semver+commit-count comparator. C reserved-keyword extractor false positives (#1148: `sizeof`/`struct`/`offsetof`) dropped before the collision pass — kernel-corpus `qualified_name_collision` rows down from 25+ to zero. `pincher vacuum` surfaces a targeted advisory when an open WAL reader pins freelist pages (#1149) instead of silently reporting 0 B reclaimed. JSON encoder now consistent across all `_meta`-bearing surfaces (#1152) so `PINCHER_DEBUG_META=1` works for error bodies too. v0.58 closed the silent-cross-project-leak class across every retrieval-shape tool (#1049–#1052). v0.57 closed the dead_code false-positive triangle's final leg via the type-info-gated resolver (#760), Python full IMPORTS+CALLS edge resolution (#856), and C same-file CALLS edges (#858 first step). After upgrading, run `pincher index <path> --force` once to pick up edges from extractor improvements. Full release-by-release history in [`CHANGELOG.md`](CHANGELOG.md); long-form rationale per item in [REFERENCE.md → Known limitations](docs/REFERENCE.md#known-limitations).
