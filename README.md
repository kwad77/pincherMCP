...
Under the hood: a single Go binary that indexes the codebase into three co-located layers — byte-offset symbol store, knowledge graph, and FTS5 full-text search — populated in a single AST parse pass from one shared `symbols` table; no duplication, no sync overhead. All three are exposed through **24 agent-callable MCP tools**, every one also reachable via the HTTP REST API at `/v1/<tool>`.
...
| **v0.82** | **Phase 4 composite #2: `plan_change` — pre-edit blast radius.** Takes a file or symbol about to be edited; composes `changes` + `trace direction=in` + `adr action=list` into one envelope `{target, blast_radius, related_adrs}` with CRITICAL/HIGH/MEDIUM risk labels. Replaces the typical 4-call pre-edit confirmation sequence. Tracked under [#1391](https://github.com/kwad77/pincher/issues/1391). | 🚧 in flight |
