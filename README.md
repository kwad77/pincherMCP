...
pincherMCP is a single Go binary that indexes a codebase into three co-located layers — byte-offset symbol store, knowledge graph, and FTS5 full-text search — and exposes all three through **9 agent-facing MCP tools** plus 13 operator/diagnostic tools on the HTTP REST API.
...
| **v0.38.0** | **Polyglot install warning + deferred dashboard panels.** `pincher init` reports per-language extraction confidence + expected savings tier so polyglot repos see realistic claims at install time ([#631](https://github.com/kwad77/pincher/issues/631)). Carved-out triangulating panels: tool-call entropy, median worth-using payload, per-tier `tokens_saved_pct` medians ([#635](https://github.com/kwad77/pincher/issues/635)). | 🚧 in flight |
