...
| **v0.55.0** | **CI hardening + exhaustive dogfood pass.** Umbrella [#681](https://github.com/kwad77/pincher/issues/681) scoped the CI half — Windows stdio-timing flakes, bench-regression noise removal, CHANGELOG-conflict elimination via per-issue stub files, workflow-isolation lint, workflow ergonomics. A full dogfood pass over the MCP + HTTP surface filled the rest: failure-as-pedagogy finished across every arg-validation / not-found / silent-coercion path; a ~2500× pinchQL variable-length-BFS perf fix (`CROSS JOIN` planner fix); an AFK-safe self-healing loop — `make install` + Windows-safe binary swap, orphaned-process reaping, monotonic project-metadata guard ([#724](https://github.com/kwad77/pincher/issues/724)). Schema unchanged at v25. | ✅ shipped |
| **v0.56.0** | **Bedrock-layer observability + a deep dogfood haul.** Phase 1 release 5 of 9. `GET /v1/events` SSE stream — dashboards and CI bots subscribe to `index_started`/`index_complete`/`binary_drift` instead of polling ([#654](https://github.com/kwad77/pincher/issues/654)). `X-Request-ID` correlation threaded through HTTP / streamable-HTTP / stdio `_meta` / logs so routers trace a request end-to-end ([#657](https://github.com/kwad77/pincher/issues/657)). `pincher project prune-stale` + `pincher vacuum` reclaim DB space from stale-but-present projects ([#732](https://github.com/kwad77/pincher/issues/732)). Diff-encoded `context` for repeat reads behind `PINCHER_DIFF_CONTEXT=1` ([#655](https://github.com/kwad77/pincher/issues/655)). Streamable-HTTP concurrent-session loadtest caught a real gzip-vs-SSE deadlock ([#687](https://github.com/kwad77/pincher/issues/687)). Auto-reindex on binary-drift respawn ([#719](https://github.com/kwad77/pincher/issues/719)). Closure-table at-scale measurement validated the depth-3 default ([#686](https://github.com/kwad77/pincher/issues/686)). Plus ~two dozen extractor/resolver/pinchQL "silent confidently wrong" bugs fixed in an exhaustive dogfood pass. Schema unchanged at v25. | ✅ shipped |
| **v0.57.0** | **Python AST + C CALLS + type-info resolver + silent-confidently-wrong sweep.** Phase 1 release 6 of 9. Schema **v26** (`pending_edges.base_type` for type-info resolver). Three structural extractor wins: full Python AST extractor with IMPORTS + CALLS edge resolution ([#856](https://github.com/kwad77/pincher/issues/856)) — the `rich` codebase went from 0 edges to 3,973 resolved edges; per-file CALLS edge pass for C ([#858](https://github.com/kwad77/pincher/issues/858)) — `trace`/`dead_code` no longer return confidently-empty results on C corpora; type-info-gated resolver kills the last leg of the dead_code FP triangle ([#760](https://github.com/kwad77/pincher/issues/760)). The bulk of the release is a 30-issue dogfood pass closing the silent-confidently-wrong family — queries that return plausible-looking but wrong results with no signal: guide `shapeAudit` now routes natural-phrasing threshold + adjective audits to the right pinchQL template scoped to Go non-test ([#921](https://github.com/kwad77/pincher/issues/921), [#923](https://github.com/kwad77/pincher/issues/923), [#924](https://github.com/kwad77/pincher/issues/924)); FTS5 sanitizer phrase-wraps CamelCase tokens ([#919](https://github.com/kwad77/pincher/issues/919)); pinchQL `RETURN DISTINCT` runs on the full match set instead of the safety-LIMIT'd subset ([#929](https://github.com/kwad77/pincher/issues/929)); ORDER BY alias resolution ([#904](https://github.com/kwad77/pincher/issues/904)); COUNT(n.property) honors NULL semantics ([#906](https://github.com/kwad77/pincher/issues/906)); CONTAINS/STARTS-WITH escape LIKE wildcards ([#885](https://github.com/kwad77/pincher/issues/885)); max_rows is a hard cap ([#900](https://github.com/kwad77/pincher/issues/900)); plus ~two dozen smaller pedagogy + classifier gaps. | ✅ shipped |
| **v0.58.0** | **Type-info-gated resolver + non-Go edge resolution + remaining engine pedagogy.** Deferred from v0.57 scope. Type-info resolver precision ([#760](https://github.com/kwad77/pincher/issues/760)). C/TypeScript edge resolution so graph tools work cross-language ([#858](https://github.com/kwad77/pincher/issues/858)). pinchQL arithmetic in WHERE/RETURN ([#928](https://github.com/kwad77/pincher/issues/928)), aggregate-alias ORDER BY ([#927](https://github.com/kwad77/pincher/issues/927)), and silent-incomplete results during binary-swap reindex window ([#925](https://github.com/kwad77/pincher/issues/925)). | 🚧 in flight |
...
- **Edge graph: Go + Python with full IMPORTS/CALLS resolution; C with same-file CALLS; other languages still graph-empty.** As of v0.57, Python joined Go on the cross-file edge resolution path ([#856](https://github.com/kwad77/pincher/issues/856)) and C gained a per-file CALLS pass ([#858](https://github.com/kwad77/pincher/issues/858) first step) — `trace`/`dead_code`/`neighborhood` now return real results on those corpora. TypeScript / Rust / Ruby / etc. extract symbols fine but produce **zero edges** — the graph-shaped tools return empty-with-diagnosis results on them ([#858](https://github.com/kwad77/pincher/issues/858)). `search`, `query` (node scans), `symbol`, and `context` work across every language; only the graph-shaped tools are affected on the remaining regex-tier languages. Cross-file edge resolution for the long tail is tracked in [#268](https://github.com/kwad77/pincher/issues/268).
...
> **Recently resolved (re-index recommended after upgrade):** the dead_code false-positive triangle's final leg via type-info-gated resolver (#760 / v0.57); Python now has full IMPORTS+CALLS edge resolution (#856 / v0.57); C corpora now produce same-file CALLS edges (#858 first step / v0.57). Plus the older fixes still live: dead_code FP triangle legs 1–3 (#423 v0.19, #493 v0.20, #565/#576 v0.21+v0.22) and polymorphic-method CALLS edge persistence from pre-v0.17 (#475). Run `pincher index <path> --force` once after upgrading to pick up the new edges. Full release-by-release history in [`CHANGELOG.md`](CHANGELOG.md); long-form rationale per item in [REFERENCE.md → Known limitations](docs/REFERENCE.md#known-limitations).
