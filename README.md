...
| **v0.58** | **Cross-language edge resolution + engine pedagogy.** TypeScript / Rust / Ruby cross-file edges so the graph-shaped tools stop being Go-only in practice ([#858](https://github.com/kwad77/pincher/issues/858) continuation). pinchQL arithmetic in WHERE/RETURN ([#928](https://github.com/kwad77/pincher/issues/928)) so audit templates can filter on derived metrics. Remaining silent-incomplete classes — supervisor-respawn reindex window ([#925](https://github.com/kwad77/pincher/issues/925)), aggregate-alias ORDER BY warning ([#927](https://github.com/kwad77/pincher/issues/927)). | 🚧 in flight |
