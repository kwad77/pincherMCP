...
## [0.87.0] — 2026-05-19 — Incremental resolve + dogfood-driven advisory hardening

**Theme — v0.87 makes the watcher's incremental tick fast and hardens the doctor advisory surface against false positives.** Headline: the resolve pass (`resolveImports` / `resolveCalls` / `resolveReads` / `resolveUsesVar`) now scopes to the files that actually changed plus their cross-file referrers, instead of re-binding the entire project's pending-edge pool on every watcher tick (#1629). v0.85's observability had measured 94% of a 1-file-change tick's cost as the resolve block; on pincher-repo a leaf-file edit now runs that block in ~87ms versus ~660ms project-wide — a 7.6× win on the common dev-loop path. Hub-file edits (whose changed-neighbourhood genuinely is the whole project) correctly fall back to the project-wide pass.

Beyond the headline: a measurement-driven Go READS pre-filter (#1622) drops predeclared-identifier pending edges at extract time; an opt-in persistent Python subprocess (#1626) amortises the per-file interpreter-spawn cost; a memory-pressure watcher backoff (#1572) skips ticks rather than risk an OOM kill; and a new FTS5 fragmentation advisory (#1612) surfaces BM25-latency degradation before the user notices a slow `search`.

### DOGFOOD

Found-and-fixed in the v0.87 window — almost entirely by running the freshly-shipped advisories + tools against the live dogfood machine:

- **FTS5 fragmentation advisory false-positived on its own first run.** #1612 shipped a 10x data/idx-row threshold; the dogfood machine's config corpus settled at 10.4x *immediately after* `rebuild_fts`, so the advisory fired on every doctor inspection. Threshold raised to 25x ([#1663](https://github.com/kwad77/pincher/issues/1663)) — clears the observed post-rebuild floor, still catches the 62.7x real-degradation case.
- **Branch-drift advisory false-positive cascade — four hops.** The #1303 advisory fired on testdata corpora, dead-path ghosts, file-not-directory paths, and nested projects. Each fix's advisory output surfaced the next false-positive class: [#1665](https://github.com/kwad77/pincher/issues/1665) → [#1667](https://github.com/kwad77/pincher/issues/1667) → [#1669](https://github.com/kwad77/pincher/issues/1669) → [#1671](https://github.com/kwad77/pincher/issues/1671). Converged when the live doctor returned only legitimate structural advisories.
- **Incremental resolve shipped, was silently a no-op, then broke a contract, then re-shipped correctly.** The #1629 slices built the scope from `seenFiles` (all walked files) — the scope matched the whole project so the resolver re-bound everything anyway ([#1676](https://github.com/kwad77/pincher/issues/1676) fixed the source to re-extracted-files-only, with a measured 3.2× tick win). The full test sweep then caught a #457 cross-file-CALLS regression; the scoping was gated off ([#1678](https://github.com/kwad77/pincher/issues/1678)) and re-enabled correctly via a pre-cascade referrer snapshot. The lesson — single-file integration fixtures pass even when scope is over-broad — is recorded; multi-file fixtures are the regression guard.
- **`guide` leaked filler words into its search recommendations.** `guide task="…resolve pass like the watcher has"` recommended `search query="…resolve pass like"` — the comparison filler "like" dragged into FTS5. Eight filler words added to the hint stopword set ([#1682](https://github.com/kwad77/pincher/issues/1682)).
- **#1477 was orphaned in a never-cut `v0.78.2` patch milestone.** The JS-AST-confidence bug it reported was already fixed; the gap was a missing end-to-end indexer-pipeline regression test (the existing test exercised the AST path in isolation). Added, and the issue rehomed.
...
- **Incremental resolve pass — the v0.87 headline (#1629).** On a watcher tick where only a handful of files re-extracted (`force=false`), all four resolvers (`resolveImports` / `resolveCalls` / `resolveReads` / `resolveUsesVar`) now re-bind only the pending edges from the changed files and their cross-file referrers, instead of walking the whole project's pending-edge pool (50k+ rows on pincher-repo). v0.85 observability had measured 94% of a 1-file-change tick's cost as the resolve block; a leaf-file edit's resolve block now runs in ~87ms versus ~660ms project-wide (7.6×), and the full tick drops from ~1015ms to ~320ms. The scope is the union of files-re-extracted and their referrers — the latter snapshotted in the main extraction loop *before* the per-file cascade deletes fire, so a hash-skipped caller's resolved edge to a re-extracted callee is preserved (the v0.16 #457 cross-file-CALLS contract). When the union exceeds 64 files — a hub-file edit whose changed-neighbourhood genuinely spans the project — the resolver falls back to the project-wide pass. New store primitives `LoadPendingEdgesByKindAndFiles` and `DeleteEdgesByKindAndSourceForSourceFiles` (handles both `resolve_pass` and `binding_pass` source kinds); all four resolvers gained a `scopeFiles []string` parameter (nil → project-wide, non-empty → scoped). Force-reindex always takes the project-wide path. Shipped across seven PRs including a gate-off/re-enable cycle to land the #457 contract correctly — see the DOGFOOD section.
...
- Branch-drift advisory false-positive suppression — four classes (#1665 / #1667 / #1669 / #1671 v0.87). The #1303 branch-drift advisory was firing on projects that don't experience meaningful drift, each class surfaced by the previous fix's output on the dogfood machine: (1) testdata corpora under `testdata/corpus/` / `testdata/__fixtures__/` / `.atrium/work/` — static fixtures, not real working trees; (2) dead-path ghosts whose source directory was deleted but whose DB row persisted — `pincher index <path>` errors on a missing path; (3) projects whose path was overwritten by a same-named file — `git -C <file>` resolves via the parent's `.git` and reports the parent's branch; (4) projects nested inside another indexed project — same parent-`.git` misattribution. Each of (2)–(4) routes to `list prune_dead=true` or the nested-project advisory (#1209), not the index-refresh remediation. The advisory now skips all four; both the MCP `doctor` and CLI `pincher doctor` copies updated per the bounded-duplication convention.
...
## [0.86.0] — 2026-05-19 — Writer-pool contention fix + PreToolUse hook converted to advisory

**Theme — v0.86 attacks the writer-pool contention v0.85's observability identified, and rebuilds the PreToolUse hook around live-dogfood friction.** Headline: `db.Store.CommitFileExtraction` (#1627) collapses four per-file transactions into one — v0.85's `pincher.index.extraction.per_phase` data showed 73% of extraction wall-clock was goroutines queued on the SQLite writer mutex (2064ms of 2840ms on pincher-repo force-reindex was lock contention, not extractor CPU). The change cuts ~2358 writer-mutex acquisitions per 786-file force-reindex pass while strengthening atomicity (per-file write is all-or-nothing across the four sub-commits).

The hook story is the v0.86 dogfood subplot. v0.85 shipped the runtime Read/Grep redirect; v0.86 ships THREE corrections from live use: (1) the hook is now ADVISORY — it never returns `continue: false`, so blocked reads never break Edit-prep workflows; (2) Grep follows the same advisory contract; (3) prose / planning files and files under 16 KB pass through silently because `context lite=true` returns nothing useful on them. The discoverability win remains (agent still sees the hint via `systemMessage` on indexed code files ≥16 KB) without the pain loop.

### DOGFOOD

Found-and-fixed in the v0.86 window:

- **Hook blocked Edit-prep Reads in a pain loop** — original blocking design (#627) assumed every Read on an indexed file was navigation. Live use showed Edit requires a prior Read, and `context lite=true` doesn't satisfy that requirement, so the agent danced between Read (blocked) → context (incomplete) → Read (blocked) on every edit cycle. Filed and fixed in [#1655](https://github.com/kwad77/pincher/pull/1655) ([#1654](https://github.com/kwad77/pincher/issues/1654)) converting to advisory mode (continue=true + systemMessage); telemetry preserved via new `Decision: "redirect_advisory"`.
- **Hook still fired on every indexed Read after advisory mode** — even passing through, the hook ran on every Read/Grep ≥4 KB. Markdown / `.planning/` / docs files are dominant false-positive class: `context lite=true` returns no useful symbols on prose. [#1657](https://github.com/kwad77/pincher/pull/1657) ([#1656](https://github.com/kwad77/pincher/issues/1656)) narrowed three things at once: Grep redirect converted to advisory (was still blocking), prose-file exemption via `isProseFile` matcher, and size floor raised 4 KB → 16 KB.
- **Hook-stats SQL missed `redirect_advisory` rows** — four queries hardcoded `decision = 'redirect'`, so once legacy rows aged out of the 7-day window every aggregate (`HookConversionRate7d`, `HookCountsByTool7d`, `HookOverrideRate7d`, `ResolveHookInvocationsForSession`) would silently zero out. Caught in the same release window before the rollover. [#1659](https://github.com/kwad77/pincher/pull/1659) ([#1658](https://github.com/kwad77/pincher/issues/1658)) unions both decision kinds.

Filed-but-not-fixed (rolled to v0.87 per the dogfood routing table): `#1572` memory-pressure watcher backoff, `#1611` parallelize resolve passes, `#1622` READS over-emission, `#1626` Python AST subprocess, `#1629` resolve re-bind incremental fix, `#1642` IMPORTS dedupe redesign, `#1612` rebuild_fts auto-advisory, `#1393` per-host benefits docs (gated on #1298 eval harness), `#640` hook conversion-rate field measurement (waits on 30-day field data).
...
## [0.85.0] — 2026-05-19 — Perf observability + usage-frequency levers (axis-A measurement, axis-B redirect)

**Theme — v0.85 ships the dual-axis observability + lever set that makes pincher's runtime cost AND adoption legible.** Axis-A (latency): per-phase / per-language / per-resolver / per-tool wall-clock timing across the indexing pipeline (#1613, #1620, #1625, #1628, #1640) — first dogfood data shows extraction is 82% of force-reindex cost and 73% of THAT is DB-write serialization, redirecting the v0.86 perf work from "parallelize resolvers" (#1611, deprioritized) to "writer-pool contention" (#1627). Axis-B (frequency): the runtime Read/Grep → pincher-tool redirect (#627 PreToolUse hook) gets a discoverability advisory in `pincher doctor` (#1635/#1636), a caller-surprised vs audit-shape zero-result split in `pincher stats` (#1633) that exposes the actionable failure rate, in-conversation adherence telemetry on every response (#1639), and `trace` reverse-direction suggestions on empty-leaf hops (#1638) so the agent has a concrete refinement instead of falling back to Read/Grep.

The release is intentionally OBSERVABILITY-FIRST: 13 of 14 entries below add measurement surfaces or close discoverability gaps; the actual perf-fix work that the data informs ships in v0.86 (#1622 READS over-emission, #1626 Python AST subprocess, #1627 DB-write serialization, #1642 IMPORTS dedupe redesign — all measurement-driven). This is the dogfood pattern at scale: ship observability, gather data, fix in the next minor with the data anchoring the design.

### DOGFOOD

Found-and-fixed in the v0.85 window by exercising the newly-shipped observability against pincher-repo + by running the autoresearch probe loop:

- **`pincher index` CLI silently discarded slog output** — pre-fix `runIndexCLI` called `log.SetOutput(io.Discard)` to silence stdlib log noise, but Go 1.25's default `slog.Default()` handler routes through `log.Default()`'s writer, so the redirect ALSO killed slog. Net effect: every observability line v0.85 was about to ship (extraction.summary, resolve_block.summary, per-resolver summaries, gc.summary) was invisible at the CLI surface — i.e. the v0.85 release would have shipped its headline feature INTO `io.Discard`. Caught before any observability-dependent perf decision was made. Fixed in [#1624](https://github.com/kwad77/pincher/pull/1624) ([#1621](https://github.com/kwad77/pincher/issues/1621)).
- **First-pass extraction data invalidated the resolve-parallelism premise** — `#1611` had been milestoned to v0.85 as "parallelize resolve passes for the obvious win." The shipped per-phase observability said: extraction is 82% of force-reindex cost (2840ms / 3445ms); resolve is 17%; GC is <1%. 4-way resolve parallelism saves at most 12% of total wall-clock — not transformative. Re-milestoned to v0.86 gated on #1629; the workstream that REPLACES it (`#1627` DB-write serialization, the 73% of extraction-phase wall-clock that isn't extractor CPU) becomes the v0.86 priority. Measurement-before-rewrite worked exactly as designed.
- **Doctor tests failed on dev machines with `~/.claude/`** — after #1636's hook-install advisory shipped, `TestDoctorReport_EmptyDatabase` + `TestDoctorReport_BinaryVersionPopulated` failed on any contributor whose home had `~/.claude/settings.json` present, because the advisory's Claude-Code detection fired in the test process. CI was green (no `~/.claude/` on GH Actions runners); the gap was only visible to humans running tests locally. New `withNoClaudeCodeDetected(t)` helper neutralizes HOME / USERPROFILE / CLAUDE_CONFIG_DIR. Fixed in [#1641](https://github.com/kwad77/pincher/pull/1641) ([#1637](https://github.com/kwad77/pincher/issues/1637)).
- **Doctor's nested-project advisory flagged testdata corpus fixtures as redundant** — the advisory naming `k8s-ops` / `terraform-stack` / `probe` as "indexed inside pincher-repo" with remediation `pincher project rm <inner>` would have broken `make corpus-test` if a user followed it. Those are pinned snapshot fixtures, deliberately indexed as standalone projects. Added `isIntentionallyNested` suppression for `testdata/corpus/`, `testdata/__fixtures__/`, `.atrium/work/` while keeping the genuine `warp-fork`-inside-`warp_rc` case firing. Mirrored to the CLI doctor.go per CLAUDE.md bounded-duplication rule. Fixed in [#1647](https://github.com/kwad77/pincher/pull/1647) ([#1644](https://github.com/kwad77/pincher/issues/1644)).

Filed-but-not-yet-fixed (rolled to v0.86 per the dogfood routing table): `#1612` rebuild_fts auto-advisory, `#1622` READS over-emission, `#1626` Python AST subprocess overhead, `#1627` extraction DB-write serialization, `#1642` IMPORTS dedupe redesign, `#1643` search exact-identifier wildcard fallback, `#1645` BY TOOL render-width box overflow, `#1646` PreToolUse hook fires too aggressively on test files.
...
## [0.84.0] — 2026-05-19 — API freeze checkpoint + cancellation contract uniformity + empty_reason orphan drain

**Theme — v0.84 is the [ADR-0002](docs/adr/0002-v1-frozen-surface.md) API freeze checkpoint.** Every PR touching a frozen surface element from this minor onward either reverts or routes to 2.x. Headline feature: schema-migration **language-scoped selective invalidation** (#1543) — when a binary upgrade ran migrations classified as `MigrationInvalidates.Languages` (not `All`), only files of the listed languages re-extract; every other file keeps its hash and skips. The earlier v0.83 routing decision to defer MCP capability expansion (#1080 notifications/progress, #1082 guide-as-Prompt) to v1.x stands — v0.84 ships the freeze plus the in-flight dogfood drain rather than the capability surface.

Beyond the headline, this release drains the `_meta.empty_reason` orphan-stamp gap from 13 catalogued constants with only 8 stamped (v0.83 baseline) → **12 stamped / 1 orphan** (only `same_file_only` remains, deferred because it requires same-file caller detection inside `trace`). Each new stamp differentiates a recoverable cause from `no_results_in_corpus` so `why_empty` can route the caller to the right fix rather than a generic "drop a filter."

And the FILE-H cancellation contract becomes uniform across every atomic tool: every standard-tier handler now returns from a pre-cancelled context in O(microseconds), not "well, except handleIndex which has to do some setup first." Pre-fix the cancellation gate would pass on fast runners and bite intermittently on Windows; this contract gap was first surfaced by a Windows CI flake during this milestone window.

### DOGFOOD

Found-and-fixed in the v0.84 window by reading shipped surfaces via pincher's own tools + by triaging CI flakes back to root cause:

- **handleIndex missed the v0.84 atomic-tier ctx sweep** — surfaced as a Windows CI flake (TestCancellation_HandleIndex_ReturnsPromptly >1000ms vs 500ms contract bound). Root cause: handleIndex relied on the deep ctx check inside `idx.Index` (indexer.go ~445); the pre-Index work (arg parse, IsBloatTrap, drift refusal, mustProject, db.Open) ran to completion before that bit. Mac and Linux still passed the bound, but the gap was real — Windows was just the first reproducer. After the fix the same test returns in ~30ms on every platform. The cancellation contract is now uniform: any atomic handler returns from a cancelled ctx in O(microseconds), period. Fixed in [#1617](https://github.com/kwad77/pincher/pull/1617).
- **`empty_reason` orphan-stamp audit** (#1603) tracked four constants that had catalog entries in `why_empty` but no handler ever stamped them. Three closed this release:
  - `low_confidence_extractor` (#1603 follow-up) — search now stamps this when `min_confidence` filters out every FTS5 candidate (`rawPreConfidenceCount > 0` AND post-filter empty), distinct from `query_too_narrow`'s filter-fix recovery. Fixed in [#1609](https://github.com/kwad77/pincher/pull/1609).
  - `unsupported_language` — search now stamps this when `language` filter targets a stub-tier extractor (currently only Haskell — confidence=0, produces zero symbols), before the relaxation verifier could rescue with the misleading `query_too_narrow` "drop the language filter" stamp. Fixed in [#1610](https://github.com/kwad77/pincher/pull/1610).
  - `stale_index` — search now stamps this when the project's `binary_version` is older than the running server and no relaxation rescues the query. CALLS/resolution edges may reflect older extraction rules; the empty result might be a false negative the user can fix by re-indexing. `next_steps` leads with `index force=true`. Fixed in [#1616](https://github.com/kwad77/pincher/pull/1616).
  - Only `same_file_only` remains orphan, deferred because it requires same-file caller detection inside `trace` (#1603's remaining open work).
- **Windows TempDir cleanup flake in `TestStartAutoIndex_*`** (#1605) — tests called `srv.startAutoIndex(...)` then asserted state and returned before the fire-and-forget goroutines completed. On Windows the indexer's `<root>/locks/` files were still held when `t.TempDir()` cleanup ran. New `Server.WaitAutoIndex()` test helper drains the in-flight WaitGroup; both tests now register it via `t.Cleanup`. Production code unchanged — auto-index goroutines remain fire-and-forget by design. Observed twice (#1576 + #1602) before the fix landed. Fixed in [#1606](https://github.com/kwad77/pincher/pull/1606).
- **Pinning regression tests for #1476** (`health` extraction_coverage cross-project leakage, reported against v0.73) — the bug is no longer present on current master (SQL already had `WHERE project_id = ?`), but the contract was untested across projects. Two new tests (Store-layer + handler-layer) pin the invariant so the multi-project surface can evolve without silently regressing it. Fixed in [#1607](https://github.com/kwad77/pincher/pull/1607).
- **JS AST parse-failure diagnostic surface** (#1477) — `parseJSWithRecovery` silently fell back from AST to regex extraction with no debug surface, leaving users debugging "why are my JS symbols at regex confidence (0.95/0.975)" with no signal. Now logs at `slog.Debug` with the file path; production logging stays quiet by default. Full plumbing of `extraction_failures` rows + `health._meta.warnings` for parser-fallback visibility remains tracked in #1477 (deferred — requires plumbing store ref into the AST extractor). Fixed in [#1608](https://github.com/kwad77/pincher/pull/1608).

Filed-but-not-yet-fixed: `same_file_only` orphan stamp (#1603 remaining open work).
...
**Theme — Dogfood-reserve patch slot + v0.83-milestone backlog drain.** Cleared the v0.83 milestone (4 issues — #1558 Go toolchain CVE bump, #1568 stdio EOF, #1590 + #1591 `context_for_task` cancellation + empty_reason) plus follow-on inner-loop hardening (#1595) caught by reading shipped composite source via pincher's own tools. Also folds in two pieces of v0.86-v0.97 scaffolding (FILE-B #1521 external comparator bench, FILE-S/T #1537+#1538 launch artifacts) that landed in the same window but had been deferred from the v0.82 megarelease. Per the routing table, dogfood-reserve releases stay small — this is the model future minors should look like rather than the v0.82 megarelease.

### DOGFOOD

Found-and-fixed in the v0.83 window by reading shipped composite source post-v0.82:

- `context_for_task` pre-dated v0.82's FILE-H sweep (#1579) — composite ran 20+ DB calls per seed (2 traces + N GetSymbol + 1 GetSymbolsForFile) with no `ctx.Err()` checks. Entry-point + per-seed checks added in the trace and neighbor loops; cancellation-test gate now covers every heavy tool (the `context_for_task` exemption is gone). Fixed in [#1592](https://github.com/kwad77/pincher/pull/1592) ([#1590](https://github.com/kwad77/pincher/issues/1590)).
- `context_for_task` seed_id-not-found stamped `EmptyReasonNoResultsInCorpus` instead of `EmptyReasonTargetNotResolved`. Conflating "missing anchor" with "empty corpus" sent callers to the wrong recovery path through `why_empty`. Mirrors v0.82's #1578 fix to `plan_change` / `investigate_failure`; the composite was missed in that sweep. Fixed in [#1592](https://github.com/kwad77/pincher/pull/1592) ([#1591](https://github.com/kwad77/pincher/issues/1591)).
- MCP stdio loop's `log.Fatalf` on stdin-EOF was killing the process before the final responses could flush — host-conformance replay (#1532 FILE-M) saw zero responses for the canonical workflow. Different surface, same shape as the #371 passive-fake blind spot recorded in v0.11.1. Now an `isGracefulStdioShutdown` classifier distinguishes clean EOF / closed-pipe from real transport errors; only the latter still fatal. Fixed in [#1593](https://github.com/kwad77/pincher/pull/1593) ([#1568](https://github.com/kwad77/pincher/issues/1568)).
- govulncheck advisory gate (#1525 FILE-F) flagged six stdlib CVEs in Go 1.25.0 — html/template XSS (dashboard renderer reach), crypto/x509 chain-build amplification (HTTPS fetch reach), net NUL-byte panic on Windows. Single-line `go.mod` bump to 1.25.10 closes all of them. CI's `go-version-file: go.mod` plumbing installs the new toolchain automatically across every workflow. Fixed in [#1594](https://github.com/kwad77/pincher/pull/1594) ([#1558](https://github.com/kwad77/pincher/issues/1558)).
- `investigate_failure` Step 2 frame-search + Step 6 callers union and `onboard_module` Step 1 / Step 3 rows.Next() loops all ran to completion after cancellation — the v0.82 #1579 sweep covered the entry-point + outer per-suspect / per-candidate loops but missed these four inner loops. Range loops get the v0.82 idiom (check at top of each iteration); rows.Next() loops check every 100 rows so per-row overhead stays near zero while cancellation latency stays bounded. Fixed in [#1596](https://github.com/kwad77/pincher/pull/1596) ([#1595](https://github.com/kwad77/pincher/issues/1595)).

Filed-but-not-yet-fixed: none. v0.83 milestone closes 4/4 plus the #1595 dogfood follow-up.
...
**Theme — Phase 4 composite suite complete + v1.0 plan refresh + dogfood-found composite hardening.** Single megarelease covering: (1) Phase 4 composites #2–#5 (`plan_change` / `audit_unused` / `onboard_module` / `why_empty`) — the suite is now 5/5 complete; (2) FILE-A through FILE-V scaffolding for the v0.83–v0.97 v1.0 plan refresh — 22 v1.0-blocker issues either shipped, scaffolded with phased rollout, or recorded as ADR-deferred decisions; (3) ADR-0002 v1.0 frozen surface + ADR-0003 plugin-extractor deferral; (4) 6 dogfood-found composite-hardening patches surfaced in the same session by reading shipped composite source via `mcp__pincher__context`. This release is intentionally large: it captures everything that landed in master between v0.81 and v0.82 rather than fragmenting across micro-tags. Subsequent minors return to one-theme-per-release cadence.

### DOGFOOD

Found-and-fixed in the v0.82 window by reading shipped composite source via pincher's own tools:

- `investigate_failure` empty-suspects branch emitted next-step `{"tool":"search","args":{"query":""}}` when only file paths parsed — invalid downstream call. Fixed in [#1586](https://github.com/kwad77/pincher/pull/1586) ([#1570](https://github.com/kwad77/pincher/issues/1570)).
- `plan_change` bare-filename target (`charge.go`) silently routed to file_path branch and returned empty — the symbols table holds full paths. Fixed in [#1585](https://github.com/kwad77/pincher/pull/1585) ([#1577](https://github.com/kwad77/pincher/issues/1577)).
- `plan_change` + `investigate_failure` stamped wrong `empty_reason` code (`no_results_in_corpus` for what's really `target_not_resolved`). New catalog code added; surfaces corrected; downstream `why_empty` recovery flow now points at the right tools. Fixed in [#1588](https://github.com/kwad77/pincher/pull/1588) ([#1578](https://github.com/kwad77/pincher/issues/1578)).
- All 5 composites + atomic `dead_code` had `_ = ctx` at the top — FILE-H cancellation contract silently violated. Entry-point + per-iteration `ctx.Err()` checks added across the suite; FILE-H test suite extended with 6 new `TestCancellation_HandleX_ReturnsPromptly` rows; the composite-exemption in `TestCancellation_HeavyToolsHaveContractTest` removed. Fixed in [#1587](https://github.com/kwad77/pincher/pull/1587) ([#1579](https://github.com/kwad77/pincher/issues/1579)).
- `why_empty` unknown-reason error response listed known reasons via map iteration — non-deterministic byte output across calls. Fixed in [#1583](https://github.com/kwad77/pincher/pull/1583) ([#1580](https://github.com/kwad77/pincher/issues/1580)).
- `onboard_module` silently dropped `rows.Err()` after partial iteration, presenting incomplete scope as complete (misleading language breakdown, missing entry-points). Now hard-errors with retry recovery. Fixed in [#1584](https://github.com/kwad77/pincher/pull/1584) ([#1581](https://github.com/kwad77/pincher/issues/1581)).

Filed-but-not-yet-fixed: [#1568](https://github.com/kwad77/pincher/issues/1568) MCP stdio `log.Fatalf` on stdin EOF kills process before flushing pending responses (caught by FILE-M host-conformance gate). [#1572](https://github.com/kwad77/pincher/issues/1572) memory-pressure-aware watcher backoff (FILE-I follow-up). [#1573](https://github.com/kwad77/pincher/issues/1573) OOM-safe indexer clean-abort (FILE-I follow-up).

Umbrella: [#1582](https://github.com/kwad77/pincher/issues/1582).
...
- **Bench baseline refreshed against v0.79 master ([#672](https://github.com/kwad77/pincher/issues/672) workstream 2 v0.79 hardening, [#1516](https://github.com/kwad77/pincher/pull/1516)):** `testdata/bench/{index,server}.bench.txt` regenerated via the new workflow (run [26054340216](https://github.com/kwad77/pincher/actions/runs/26054340216)) — Linux AMD EPYC 7763, `-4` GOMAXPROCS, `-benchtime=10s`. Old baseline was PR #173 era (pre-v0.71, module path still `github.com/pincherMCP/pincher` before the rename); 8 feature minors of unmeasured drift. New baseline shape: Cold index per-file got more expensive as we added languages (Scala/Lua/Zig/Elixir/Dart/R/HTML/XML extractors, branch stamping, USES_VAR resolution, structured `_meta.warnings_v2`); Incremental NoChange and every Server handler got dramatically faster (#1315 watcher hot-path -85% allocs, #1322 ApproxTokens cheap default cut 60% of authenticated-handler allocs). `make corpus-bench` now gates against v0.79 ground truth — the "no perf regression >5%" criterion is enforceable for the first time since #173.
...
## [0.78.1] — 2026-05-18 — Force-pass perf + watcher serialize + schema-migration invalidates audit

Patch release driven by #1474 user report ("USES_VAR shows in --json-summary but absent from DB" + "force=true rejected with lockfile contention" after schema-migration auto-restart). CPU profiling reframed the investigation — the force-pass hot path was 42% SQL parsing in `DeleteSymbolsForFile`'s per-symbol DELETE loop + 42% `os/exec.Cmd.Start` in `git rev-parse` invocations bypassed by the force=true branch. Two targeted perf PRs ([#1495](https://github.com/kwad77/pincher/pull/1495) + [#1498](https://github.com/kwad77/pincher/pull/1498)) cut both: subquery bulk deletes drop SQL parse share from ~42% → ~10% (6× reduction in parser pressure), HEAD-content cache skips redundant `git rev-parse` when `.git/HEAD` is unchanged, watcher reindexes serially so user `force=true` against any idle project wins the lockfile race immediately. Wall-clock force pass on pincher-repo: 6.77s → 6.27s median (3 runs each, ~7% faster). Plus the structural audit groundwork via [#1502](https://github.com/kwad77/pincher/pull/1502) classifying all 32 schema migrations by their invalidates scope (27 Nothing / 5 All) with the doctor surface live — the gate that suppresses `binaryDriftForce` when migrations applied this startup union to Nothing is the v0.80 follow-up. Plus README / REFERENCE Known limitations audit ([#1501](https://github.com/kwad77/pincher/pull/1501)) catching three stale claims, the `retry_rate` → `zero_result_rate` metric-honesty rename ([#1499](https://github.com/kwad77/pincher/pull/1499)), and Ansible USES_VAR diagnostic counters + regression tests ([#1500](https://github.com/kwad77/pincher/pull/1500)) since the reported #1479 symptom did not reproduce against canonical Ansible fixtures pending a user repro corpus. **v0.78.0 binary catch-up:** [#1085](https://github.com/kwad77/pincher/issues/1085) MCP `notifications/message` surface + [#1081](https://github.com/kwad77/pincher/issues/1081) Phase 2 multi-root auto-index actually landed in master before the v0.78.0 release-prep merge — the v0.78.0 binary ships them; this section is the CHANGELOG catch-up.
...
## [0.78.0] — 2026-05-18 — Dogfood drain: prune-cascade transient state + Claude Code worktree double-index

Two more produce-but-not-persist-shape dogfood bugs drained from the overnight filing batch (#1473-#1482). **(1) `list prune_dead=true` transient counts ([#1473](https://github.com/kwad77/pincher/issues/1473)):** the prune CASCADE deletes files/symbols/edges rows whose paths live under pruned project paths; where files belong to multiple projects (parent + worktree sub-project sharing physical paths), the cascade temporarily reduces the JOIN counts the same SELECT then reports. A user reading the response pre-fix would reasonably think the prune nuked their indexes (counts collapsed 80-95% mid-cascade, recovered within ~60s after the watcher reindexed). Now the response carries `_meta.counts_may_be_transient: true` + a warning when the prune list is non-empty. Option (c) from the issue — preserves the existing response contract; the other two alternatives required a contract change or synchronous indexer coordination. **(2) Walker descends into `.claude/worktrees/` ([#1475](https://github.com/kwad77/pincher/issues/1475)):** Claude Code's per-feature-branch sibling working tree convention got walked under the parent project's project_id, producing duplicate `file_path` rows across project IDs and inflated symbol/edge counts on the parent (dogfood case: parent project files jumped 84 → 335 after a worktree dir appeared). Fix: `.claude` added to `skippedDirs` so `gocodewalker.ExcludeDirectory` skips the entire subtree before any file emits. Pragmatic name-based fix — the principled approach (detect any nested `.git` entry as a project boundary) needs gocodewalker per-dir callback support and is left for a later slice. **Roadmap roll-forward:** four issues stay deferred — #1080 / #1085 notification-surface (SDK plumbing), #1081 Phase 2 multi-root auto-index (envelope contract change), #1298 cross-comparator pricing (multi-tool install), #1333 plugin surface (Phase 5), #1337 multi-platform install validation harness, #1474 watcher poll backoff + #1479 Ansible USES_VAR persistence (paired — #1479's race-hypothesis depends on #1474 first). v0.78 banks the two tractable wins from the overnight filings; the rest move forward as cohesive slices.
...
## [0.77.0] — 2026-05-18 — Dogfood-found extractor regressions: Bash + Markdown both produce zero edges in production

Two real produce-but-not-persist regressions discovered via this session's dogfood pass on pincher-repo and homelab-iac. Both shipped in earlier v0.71/v0.72 releases (#1341 Bash CALLS, #1343 Markdown REFERENCES), both had passing unit tests asserting incorrect shapes that didn't round-trip through the resolver, both dropped 100% of their edges in production. **(1) Bash CALLS:** `bashEdgeFromName` returned `""` for top-level invocations with a code comment claiming "the indexer treats that as the file-scope edge attachment point" — but the indexer has no such handling, and 100% of top-level Bash CALLS were dropped at indexer.go:706. Fix: bash extractor emits a per-file Module symbol (matches Go's per-file Module convention at extractor.go:565) with `QualifiedName = filename-without-ext`, and `bashEdgeFromName(scope="")` returns that module name. The indexer's `nameToID[module]` now resolves to the Module symbol's ID and the edge persists ([#1482](https://github.com/kwad77/pincher/issues/1482)). **(2) Markdown REFERENCES:** the link walker passed `result.Module` (= filename basename like `"README"`) as `selfModule` to `canonicalDocsLinkTarget`, so `[Configuration](#configuration)` emitted ToName=`"README.configuration"`. But Section symbols are built from the H1-heading hierarchy (`# Project` → QN root `"project"`), so the actual Section's QN was `"project.configuration"`. Shape mismatch → 100% of intra-doc REFERENCES edges dropped at the resolver. Fix: derive `selfModule` from `headings[0]` (the first heading's slug). Sections and edges now share the H1-rooted QN convention ([#1481](https://github.com/kwad77/pincher/issues/1481)). Both bugs share the same root anti-pattern: **the unit tests asserted the extractor's output shape without round-tripping through the resolver**, so the per-extractor test suite stayed green while production lost every edge. Both fixes ship with new integration probes (`TestIndex_BashTopLevelCallsToLocalFunc_PersistedToDB_1482`, `TestIndex_MarkdownIntraDocREFERENCES_PersistedToDB_1481`) that exercise the full extract → resolve → persist pipeline against the production-failure shapes. **Roadmap roll-forward:** four items moved out to align with the actual v0.77 slice — the notification surface work (#1080 `notifications/progress`, #1085 `notifications/message`, #1081 Phase 2 multi-root auto-index + `_meta.auto_indexed`) rolled to v0.78 to ship as a cohesive slice once `ServerSession.Log` + `ServerSession.NotifyProgress` wiring lands; #1479 (Ansible USES_VAR extracted-but-not-persisted) + #1474 (watcher stays in 2s active-poll after schema migration) also rolled to v0.78 since #1479's race-hypothesis depends on #1474 being fixed first.
...
## [0.76.0] — 2026-05-18 — MCP roots multi-root selection + USES_VAR allowlist drift fix

Two real correctness fixes from this session's dogfood pass. **(1) MCP roots/list multi-root iteration with bloat-trap clearance ([#1081](https://github.com/kwad77/pincher/issues/1081) Phase 1):** pre-fix `detectRoot` used `result.Roots[0]` unconditionally — if an MCP host advertised `/` or another bloat-trap default at index 0 followed by the actual project at index 1, pincher set the session root to the bloat-trap path. Post-fix `pickSessionRoot` iterates every advertised root and filters by `parseFileURI` + `IsBloatTrap(hookMode=true)` (project-marker requirement: `.git`, `go.mod`, `package.json`, etc.); first cleared root wins. Single-root hosts and the CWD fallback behave identically to pre-fix. Phase 2 (background auto-index of additional cleared roots + `_meta.auto_indexed` field surfacing) deferred to v0.77 to ship alongside the notification-surface slice (#1080 / #1085). **(2) `USES_VAR` edge-kind allowlist drift ([#1480](https://github.com/kwad77/pincher/issues/1480)):** `#1380` (v0.71 Ansible Jinja-substitution dataflow edges) shipped the extractor + resolver but missed updating two hardcoded `knownEdgeKinds` allowlists (`internal/cypher/engine.go:494` query MATCH validation, `internal/server/server.go:7133` trace handler `kinds=` filter). Pre-fix `MATCH (a)-[:USES_VAR]->(b)` returned 0 rows AND a misleading `"not recognized"` warning even when USES_VAR rows existed in the DB; the warning conflated allowlist-miss with no-rows-in-DB. Post-fix `USES_VAR` is recognized in both query and trace allowlists; the unknown-kind warning text enumerates it so agents who typo discover it exists. Pinned by `TestExecute_USES_VAR_EdgeKindRecognized` (positive) + `TestExecute_UnknownKindWarning_NamesUSES_VAR` (cross-check). Same drift class as #1328 (JS AST registry / health-label / docs drift) and #1477 (filed in this session — #1328 still drifting in v0.73) — symptomatic two-line patch here; the structural fix (single source of truth for edge kinds — registry that indexer + cypher + server all import) is tracked separately.
...
## [0.75.0] — 2026-05-18 — Docs completeness dogfood-3: deployment guides + host tutorials + README framing audit

Third dogfood-style release dedicated to documentation completeness (pattern matches v0.65 dogfood-1 and v0.68 dogfood-2; #668 Phase 3). Three coherent doc PRs land together. **Deployment guides:** `docs/deployment/` grows from one page (helm) to four pages plus a hub — `docker.md` (single-container + docker-compose + indexing host directories + resource sizing on the pincher-repo corpus), `systemd.md` (Linux user service + drop-in override + journalctl recipes + linger config), `homebrew.md` (macOS+Linux tap + `brew services` + auth env file + tap-maintainer manual-bump notes), and `README.md` deployment hub with decision matrix + common ops concerns. Replaces the previous "only helm.md exists; everything else lives in packaging/README" state where install-path quickstart was scattered. **Host tutorials:** `docs/tutorials/` grows from two pages (claude-code + cursor) to five — `zed.md`, `codex.md`, `jetbrains.md` ship modeled on the same 6-step shape (install → index → wire → try it → watch savings → host-specific notes). Closes the v0.65 acceptance gap "at least one tutorial covering the agent-integration golden path" for four additional first-class hosts. **README leading-paragraph audit:** two framing corrections per the v0.73 vocabulary discipline — "the retention is routing" → "the retention is the planning-loop surface" (pincher = foundation, `_meta` = enabler, the LLM in the host's loop = router); "Sourcegraph / OpenGrok / IntelliJ" → "Conventional code-search tools" (no-comparison-docs-pre-1.0 discipline). The expanded `_meta` field list in the leading copy now names `next_steps`, `empty_reason`, `warnings_v2` alongside the legacy `capabilities` / `complexity_tier` / `X-Request-ID` triad. **Roadmap roll-forward:** #1085 (MCP `notifications/message` for extraction failures + slow queries + drift warnings, "judgment call on signal-to-noise threshold") moved out to v0.77 so it ships in one coherent slice with #1080 (`notifications/progress` for long-running tools), since both are notification surface and signal-to-noise design questions should be made together.
...
## [0.74.0] — 2026-05-18 — Phase 4 prep: composite-tool roadmap + use-case pricing framework + bench composite-score rubric

v0.74 is the falsifiable-measurement slice. Three coherent artifacts ship together so the Phase 4 composite-tool work (v0.81 → v0.90) lands against a published methodology rather than an after-the-fact rationalization. **(1) The composite-tool roadmap** names the five candidate composites (`investigate_failure` v0.81 / `plan_change` v0.82 / `audit_unused` v0.83 / `onboard_module` v0.84 / `why_empty` v0.85) with today's atomic-call sequence each replaces and the contract invariants every composite must hold (additive only, no internal MCP round-trips — the Stoa anti-example — single envelope, per-composite `_meta.diagnosis_v2` codes, mandatory `empty_reason`, idempotency). **(2) The use-case pricing framework** names four serious-use-case scenarios (find-dead-code N=20, why-test-failing N=10, blast-radius N=10, orient-on-module 3-contributor-rubric) plus one negative-control (format-this-file) that every composite gets priced against — token cost + wall-clock p50/p95 + accuracy via manually labeled reference set. Establishes the per-doc citation policy: every claim in `composite-tool-roadmap.md` / `loop-leverage-layers.md` / `meta-envelope-contract.md` about composite-vs-atomic deltas must cite a specific Scenario N row. No abstract "5-10 calls collapse" claims permitted. **(3) The composite-score rubric** (`cmd/pinch/bench_composite.go`) ports Stoa's autoresearch-score pattern with pincher-domain weights — accuracy 10× (correctness floor), round-trips-saved 5× (composite-collapse IS the headline), latency p50/p95 0.05/0.02 (median dominates tail for interactive UIs), per-token 0.001 (large absolute numbers, weight stays small). Default-weights ranking pinned by `TestComposeScore_DefaultWeightsRanking` so weight tuning shows up in PR diffs. **The closed precedent:** `_meta.estimated_cost_usd` (#1396) closed as wontfix — pincher stays tokens-only per the foundation/enabler/router vocabulary discipline; routers (Ordo etc.) multiply `_meta.tokens_used` by their own model's cost-per-Ktoken host-side. Pricing tables move; the trivial multiply belongs at the router level. **Roadmap roll-forward:** four v0.74 items moved out to align with available implementation runway — #1081 MCP roots/list → v0.76; #1080 MCP notifications/progress → v0.77; #1298 bench harness §2 (cross-comparator implementations) → v0.78; #1333 plugin surface → v0.85 (Phase 5 territory).
...
## [0.73.0] — 2026-05-18 — Documentation frame + universal regex-tier sweep (Swift/Kotlin/C#/PHP/C/C++ all to 0.85)

v0.73 is a two-axis release. **(1) The Phase 3 docs frame lands:** the `docs/integrations/` directory ships with `loop-leverage-layers.md` and `meta-envelope-contract.md` — the conceptual model the rest of the Phase 3 docs build on (pincher = foundation, `_meta` = enabler, the LLM in your host's loop = router; three leverage layers — hook + `_meta` + composite — stack rather than substitute) plus the per-field contract that v1.0 will freeze. The migration guide at `docs/migration/v0.4-to-v1.0.md` catches up to v0.72 (schema v33, scope-to-session sweep, BREAKING `last_indexed_branch` rename, pinchQL `IN` / `COUNT(DISTINCT)`). The GitHub Pages landing page gets two passes — a framing backfill (dropping "routing layer" language that violated the foundation/enabler/router framing discipline) and a full restructure around the three-leverage-layer narrative with a concrete worked example (same investigation shown three ways: ~45K tokens atomic-only / ~3.2K with `_meta` branching / ~2.8K with hook+composite) plus a per-host `pincher init --target=<host>` matrix covering all nine first-class init targets. The comparison table is deleted (violates the no-comparison-docs-pre-1.0 framing discipline). **(2) The universal regex-tier sweep:** every remaining 0.70-tier single-language extractor promoted to 0.85 stable regex in one cycle — Swift (#1450), Kotlin (#1457), C# (#1459), PHP (#1461), C/C++ (#1463). Each adds the modern type kinds the language's grammar declares as first-class today (Swift: struct/actor/enum/init/subscript with @attribute prefix and `class func`/mutating modifiers; Kotlin: data class/sealed class/object/companion object/enum class/interface/fun interface with @Annotation prefix; C#: record/record class/record struct/struct/enum with [Attribute] prefix and file-scoped/partial/required modifiers; PHP: interface/trait/enum/#[Attribute] with final/readonly modifiers and flexible modifier ordering; C/C++: class/struct/union/enum/scoped enum class with inheritance lists). **Framework gains:** `enumRE` matches now set `currentClass` so enum methods scope as Method with the enum as Parent (mirrors the #819 `interfaceRE` shape); `classRE` matches now suppress `funcRE` for the same line so C# 9+ records (`public record Person(string Name, int Age)`) don't mis-emit as Function; the shared generic type-parameter clause regex tolerates one level of nesting (`<T : Comparable<T>>`). The Swift / Kotlin / C# / PHP / C / C++ AST-tier extractors are tracked separately for v0.85+ (parallel to #1182 Rust AST / #1183 Java AST), gated on the v0.79 #1389 cross-language false-positive sweep deciding whether regex tier passes the v1.0 bar.
...
## [0.72.0] — 2026-05-17 — Scope-to-session sweep finishes + pinchQL IN / COUNT(DISTINCT) land + 30 dogfood-driven fixes

v0.72 completes the strict-cross-project guard (#1232, v0.66) follow-through across the retrieval-shape surface. Pre-cycle, the guard could fire wrongly because `symbol` / `context` / `neighborhood` / `trace` all looked up via an unscoped `GetSymbol` first — when the same id lived in both the session project and a fork, the fork's row could win the lookup and the strict guard would then reject the call with "exists only in project X" even though the symbol DID also live in the session. v0.72 fixes that across every retrieval-shape tool (#1408 symbol/context, #1425 neighborhood, #1431 trace) — the canonical `search → symbol` workflow now stays sane on any host that has indexed forks of the same source tree. **pinchQL grows two long-standing surface gaps:** `WHERE col IN [a, b, c]` (#1439, the v0.8.0-era OR-chain workaround) and `COUNT(DISTINCT n.prop)` (#1437) both become first-class. **Two extractor shadow bugs** drained the same family of dead_code FP candidates that v0.21's #565 binding-pass introduced: Go parameter / receiver / body-local shadows now suppress phantom READS edges (#1423) and CALLS edges (#1429) when a local name matches a project Function. **TS nested-function var-scope correctness** (#1422) replaces #1375's single-slot most-recent-function tracker with a stack — sibling inner functions in Next.js App Router page.tsx files no longer collide at module level via the qualified_name_collision dedup. **Doctor evidence improves**: extraction_failures rows now carry `binary_version_at_failure` (schema v33, #1421) so operators can tell fixed-since-this-binary rows from still-recurring without cross-referencing CHANGELOG; the project-arg filter (#1401) and tiered-match (#1404) make doctor usable on multi-project installs. **One BREAKING (pre-1.0):** `projects.current_branch` JSON tag renamed to `last_indexed_branch` (#1388) across every response that carries a project record. **New tools:** `pincher verify` integrity-check subcommand (#1399) and `pincher init --target=antigravity` (#1368) for Google Antigravity. **Quality-of-life:** TypeScript receiver-type resolver (#1177) closes the v0.61 TS receiver-type stack, eliminating the dead_code FP family on TS code; `changes` accepts git revspec operators (#1433); `init` rich-error args round-trip Windows paths correctly (#1435); `context_for_task` AND→OR fallback (#1440) mirrors search; `neighborhood` token-aware truncation + `fields` projection (#1442) ensure large files surface partial results with continuation hints instead of blanket errors.
...
- `doctor` extraction_failures rows now carry `binary_version_at_failure` so readers can distinguish "fixed-since-this-binary" rows from "still recurring on the running binary" without cross-referencing CHANGELOG by hand. Schema v33. (#1421)
...
- `init` rich-error `next_steps[].args` no longer breaks JSON when `project_path` contains backslashes (Windows paths). Pre-fix the args were string-templated, leaving `D:ClaudeCodeepo` embedded as invalid JSON escapes; now marshalled via `json.Marshal` so every shell escapes correctly across platforms (#1435).
